...
- **Cross-wave reasoning is the value:** `computeConflictMap` is wave-agnostic; feed it `(candidate) ∪ (queued+in-flight)` to answer "can these waves run side-by-side?" The Reviewer's own diff-against-declared-globs check is the runtime guard that holds declared globs; `files-drift` computes that same check for a markdown issue file, but no skill, agent definition, or driver ever invokes it.
