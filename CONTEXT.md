...
The human-decided resolution a **Disclosure** must reach before the wave archives: `resolved-in-slice · scope-extension · filed:<id> · dropped:<reason> · upstream:<ref>` — `open` until decided, and `wave-close` refuses the Archive phase while any Disclosure is `open` (ADR-0027). `filed` records *existence*, not wave-readiness: a bare tracker issue with a provenance line, decorated later via `to-issues`. Several Disclosures may share one filed id — a **thematic bundle** issue; own-ticket vs bundle follows the triviality default (own ticket only for a mechanism defect with observed consequence; ADR-0027 Amendment 2026-07-31), and a bundle accepts appends only while still bare. `upstream` is the exit for a find about **the toolkit itself** rather than about this repo — handed to flotilla's own tracker, where `filed` would name the wrong one and `dropped` would read as discarded; its `<ref>` is free-form (URL, number, phrase) and only emptiness is refused, and the `report` skill's filing is what ordinarily supplies it (ADR-0027 Amendment 2026-09-04). Deliberately the same word as the row dispositions (park, abandon): a scripted, human-decided exit for an exception — never automatic.
_Avoid_: resolution (vague), auto-file (rejected — the write is human-decided; only the *enforcement* is mechanical), bundled-id (the shared filed id *is* the bundle — no second id concept), upstream-id (the upstream reference is deliberately free-form, not an id in any scheme this repo owns).
...
The need a verify command declares beyond the worktree floor a dispatched agent runs on — one of a closed set of three, each naming a sandbox *dimension* and never any harness's knob: `writes` (named paths outside the worktree), `network` (named hosts), `host` (the machine itself — a daemon socket, a simulator, a device; the one class that cannot be narrowed). Declared by the consumer at setup, on the command, never discovered by the agent; provided by the harness binding setup scaffolds (`writes`/`network` tracked, `host` operator-local — by design, a tracked command exclusion is a host-escape for every future agent); measured at setup by running the command once *inside* the sandbox; carried into both briefs as data, so the **Worker** and the **Reviewer** — which re-runs the same commands and meets the same wall — read one expectation. A gate whose requirement is undeclared or unprovided is *withheld*, never forced: **a dispatched agent never escalates its own permissions**, attended or not — the acceptance criteria that gate would have backed land `deferred` through the Reviewer's valve (trigger `capability-gated`), captured as a **Disclosure**, and the human act moves from a blocking click to a **Disposition** (ADR-0049). An undeclared need an agent meets is not a licence but a disclosure that names the command — the next wave carries the declaration (the ADR-0041 one-round boundary).
_Avoid_: sandbox exception / unsandboxed (the knob, not the need — the engine never learns it), permission (the allowlist answers *may this command run*; a requirement answers *what does it need to run*), grant (a **Scope extension** is a mid-wave decision about paths; a requirement is standing config about capability), escalation (the act the rule forbids, never a thing an agent holds).
...
**Terminal wave**:
A **Wave** every row of which sits in a terminal **Fine state** (`pr-created`, `approved`, `failed`, `abandoned`, `parked`) — no **Worker** or **Reviewer** can still be running for it. It is the condition `wave-close`'s load gate checks before anything else, and the one condition under which the **Sweep** treats the wave's own residue (its review refs, its composed drivers) as no longer live and sweeps it at the wave's *own* close rather than one wave later (ADR-0042 Amendment 2026-09-08). Row membership alone never makes residue live; row state does.
...
**Probe checkout**:
The detached checkout a **Reviewer** makes outside the repository to exercise a row's outcome — outside by necessity, since the harness denies agent-configuration files at any depth of an in-repo checkout — named with the stamp `flotilla-probe-<wave-slug>-<row-id>-i<iteration>`, which is the checkout directory's own basename, never a parent of it. It belongs to exactly one row and one iteration, and is live only while that row reads a running state (`dispatched`, `re-dispatched`, `reviewing`, `report-in`, `verdict-in` — the last two the states a resume reconstructs, issue #991) at that iteration; the **Coordinator** collects it through the **Sweep** when it routes the verdict, recognised by the stamp rather than by a **Containment root** — the Reviewer never removes it (ADR-0042 Amendment 2026-09-23, Correction 2026-09-25).
...
**Survivor set**:
What physically remains under a worktree after a removal attempt has **exhausted its permissions** — every entry tried once, only the refused ones left standing (ADR-0042 Amendment 2026-09-08). The evidence the exhausted reading is judged on, and (ADR-0042) disclosed on the report entry rather than discarded after the verdict.
_Avoid_: leftovers, residue (unqualified — residue spans both this and **Unaccounted**); the remainder of a delete that stopped at its first refusal (that is an aborted tree, not a survivor set — nothing can be judged from it).

**Manual recovery**:
The exhausted removal's handoff payload on the report entry — a why-message plus the exact operator commands. The sweep's half of "accounting, never removal" for the residue it can see (ADR-0042). It is executed by whoever runs *outside* the harness sandbox — the **Operator** interactively, the runner script around a headless **Pulse** — and never by an agent, which owns no sandbox switch (ADR-0049); a Pulse only reports it as hygiene owed, advisory, never as an **Ask** (ADR-0042 Amendment 2026-09-08).
...
**Guard**:
A check that reads a *text* — a Bash command about to run, a shipped file, a skill's prose — and answers with a blocking verdict, at the hook or drift-spec **Enforcement Tier**. Ten ship: two `PreToolUse` hooks and eight drift specs. Distinct from the DoR **Gate**, which inspects an artefact's structured fields and may defer.
_Avoid_: linter (it blocks an invocation, it does not annotate source), validator (overloaded with the DoR), scanner (a Guard's *mechanism*, never its role).

**Abstention**:
A **Guard**'s third answer: it read its subject and reached no verdict, naming the shape it could not parse instead of asserting a defect. A statement about the Guard, never about the subject — a Guard never words an Abstention as a finding, and never lets a decided answer borrow an Abstention's hedging or the reverse (ADR-0052).
_Avoid_: deferred (a **Gate** defers — its data source was absent and the check never ran; an Abstention ran and failed to conclude), unknown / inconclusive (both read as a property of the subject).

**Resolution bias**:
The direction in which a **Guard** turns an **Abstention** into an action — block or pass — declared in the Guard beside its subject, and chosen per subject rather than fixed across the corpus: `conv12-guard` blocks (standing still is cheap), `echo-guard` passes (its traffic is prose full of shell metacharacters). Saying is mandatory, blocking is discretion (ADR-0052).
_Avoid_: fail-open / fail-closed as the *name* (they name the two values, not the declared property), default (nothing is defaulted — an undeclared bias is a defect).

**Unmodelled set**:
The constructs a **Guard** states it does not model, written beside its scanner rather than discovered by the next false refusal — the echo-guard's own "named rather than assumed away", promoted from a comment to a duty (ADR-0052).
_Avoid_: residual (ADR-0034's **residual form** is the shrunken prose left after a **Promotion** — a different thing, one group over), limitation / known issue (both read as an apology; the Unmodelled set is part of the contract).
...
- A **Guard** answers in three kinds — decided-block, decided-pass, **Abstention** — and no kind borrows another's wording; its **Resolution bias** decides what an Abstention *does* and its **Unmodelled set** says what it never read, both declared in the Guard and never in the checked text (ADR-0052, on ADR-0043's guard-side ground). The **Reviewer**'s `questions-blocking` and the **Gate**'s **Deferred (gate outcome)** are the same discipline at the other two layers; Guards were the last family without it.
...
- **"deferred"** gained a fourth neighbour with the Guards. **Resolved (ADR-0052):** a **Gate** *defers* — its data source was absent, the check never ran; a **Guard** *abstains* — it read its subject and reached no verdict. Both are non-verdicts; only the second one looked. Never write "the guard deferred".
