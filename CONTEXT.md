...
**Goal**:
A named finish line for a body of work, bound to one config-selected native target container (GitHub Milestone · Linear Project · MarkdownFs goal file) whose issue membership is the goal's member set — the container-role binding is config-authoritative per consumer, set at setup time, fail-loud when absent, never a built-in assumption (ADR-0044). Members are **issues only**, joined by curation (an **Operator** act), whatever their origin — PRD slices, filed Disclosures, hand-filed issues, or bare tickets cut by the `goal` station itself; a **PRD** is never a member (it "belongs" to a Goal only derivedly, through member slices). A Goal grants **sight, never permission**: membership informs planning and adds no execution path — an issue enters a wave because it is eligible and passes DoR, never because it is in a Goal.
...
**Frontier**:
A **Goal**'s derived open remainder — the members not yet natively closed, each read as one of `in-motion · actionable · blocked · unready` (claimed · could-start-now · unresolved dependency · bare, awaiting sharpening). Derived, never written — the coarse-bookend discipline; completion is literally *the frontier is empty*, and even then the `goal` station only reports it: closing the container is the Operator's act in the tracker, not a verb — the station owes accounting, never the declaration.
