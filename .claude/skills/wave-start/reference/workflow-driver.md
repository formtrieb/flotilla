...
The Scribe is the one role in this pipeline that is **not** worktree-isolated: it runs in the session cwd. **The measured fact this file and [wave-shared Convention 13](../../wave-shared/reference/convention-13-one-bash-call-per-step.md) state identically: a dispatched agent's cwd is reset to its dispatch root before *every* Bash call** — the worktree root for a worktree-isolated role, the session cwd for one (like the Scribe) without isolation. So the Scribe brief below **opens with a bare `pwd`** and compares it against the compose-time `REPO_ROOT` literal rendered into its own text — no `cd`, and no shell variable — **passes every path argument absolute**, and **treats a cwd mismatch as a finding to report, never as something to `cd` around.** That precondition belongs to the Coordinator, which dispatches from the repo root and fills `REPO_ROOT` from that same root (`git rev-parse --show-toplevel`).

Full derivation — the retired `cd`-then-engine-call split and why it only looked safe, both configured `engine.cli` forms (ADR-0032) and how each fails differently on a wrong dispatch cwd, and the four-cwd npm/npx measurement that established this (issue #355, 2026-07-31): [`evidence/workflow-driver.md` § The Scribe's cwd](../evidence/workflow-driver.md#the-scribes-cwd--a-precondition-observed-once-never-a-cd-carried-forward).
...
Two call sites in this pipeline open a PR through the engine's find-before-create seam (`host-pr create`, ADR-0019) — the Worker's own Termination step 3 below, and the terminator's CALL 1 (`start-mechanics.md` step 7c).
...
**Last-writer-wins is about the value the caller hands over.** The Worker always passes `--title`, so on the Worker's own call the title it composes is the title that lands. The terminator does not: `route-tuple` on a REUSE **preserves the live PR title byte-identically** and re-writes only the body, unless `--title` is passed to it — *on reuse the body is re-written from the composed render and the title is preserved unless `--title` is passed.*

**What reaches the default branch is decided at landing (ADR-0053).** Under `landing.commitMessage` `pr` (the default, and what an absent key means) the landed commit is the PR's title and body, on any commit count, and the Worker's commit messages never reach the default branch — except under a merge queue, which composes its own commit, and `--method rebase`, which lands the Worker's commits themselves; under `host` the repository's squash setting decides. The Worker brief says so beside its title rule.

Full derivation — the false-success shape this contract forecloses, why the terminator had to stop overwriting the live title, why a correctly-composed body can never legitimately trigger the refusal, and the trap in the refused payload still carrying the PR's URL: [`evidence/workflow-driver.md` § PR-open reuse-refusal](../evidence/workflow-driver.md#pr-open-reuse-refusal--what-host-pr-create-guarantees-and-refuses-the-caller).
...
5. **Free-form brief text never has to be hand-escaped into a JS literal.** The
   ...
   parse. That class is now closed by construction.
...
Full derivation — the two live incidents behind constraints 3 and 5 above (a fabricated anchor SHA that four Workers independently caught, and the W17-F1 script-parser failure): [`evidence/workflow-driver.md` § Composition constraints](../evidence/workflow-driver.md#composition-constraints--the-live-occurrences-behind-the-numbered-properties).
...
**The token is a STATIC LITERAL in the shipped script — it is NOT compose-time-filled.** `compose-driver` fills six constants and the `ISSUES` array and nothing else; `HUMAN_GATED_WORKERS` is none of them. `Worker` is a config-governed enum (ADR-0007), so the array carries the consumer's set, written out in the shipped script defaulting to the engine's `HUMAN_GATED_WORKER` (`tools/wave/src/wave-md-rw.ts`, whose `humanHeldRowIds` owns the same predicate engine-side) — a script cannot `import` it, having no filesystem and no local modules, exactly as it cannot import the two `*_SCHEMA` literals beside it. What keeps the copy honest is a pin, not a fill: `tools/wave/src/skill-schema-drift.spec.ts` extracts the array and asserts it equals the engine constant, so a re-spelling on either side fails loud. A consumer whose vocabulary differs edits that line in its own copy of the script.
...
`isolation: 'worktree'` is a REQUEST for a worktree, not a promise of a FRESH one. When the harness retries a dispatch it re-runs the agent in the same checkout the previous attempt was already working in, so a first-iteration Worker can arrive to uncommitted edits, untracked new files, and an already-created wave branch, none of it recorded anywhere — and the brief's remedy is *discarding is the default*, with adoption permitted only behind a **recorded** line-by-line review (every inherited line against every acceptance criterion, any resting measurement re-run, the full verify gate before anything is touched) written into `judgmentCalls` and mirrored into `reviewerFocusItems`: *no record, no adoption.*

Full derivation — the live occurrence (row 716) that showed this needed a brief clause rather than an agent's good judgment, the two dead ends the setup had no instruction for (a refused reset, an already-existing branch) and their remedies, the untracked-leftovers accounting and the `git clean`/`git worktree remove` ruling, and how this case is pinned apart from a re-dispatch: [`evidence/workflow-driver.md` § A harness retry reuses the worktree](../evidence/workflow-driver.md#a-harness-retry-reuses-the-worktree--inherited-work-in-progress-in-iteration-1-issue-731).
