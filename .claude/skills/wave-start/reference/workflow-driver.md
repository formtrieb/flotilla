...
engine's sidecar-write-gate `notice:` (§Why the shipped driver's `WORKER_REPORT_SCHEMA` drops `anyOf`,
...
Both are threaded into `workerBrief()` below: whenever `issue.iteration > 1` it renders a DIFFERENT `## Workspace setup` block (this doc's own §Re-dispatch workspace setup, inline in the script). The `issue.iteration === 1` rendering — the default, and the one the schema-drift spec's neighboring pins exercise — is NOT the bare block it once was: it carries its own inherited-work-in-progress clauses (§A harness retry reuses the worktree, immediately below), which answer a DIFFERENT case from this one and must not be folded into it.

**A third trap lives in the tracking-free checkout itself: it can HALF-APPLY under the harness write-deny (issue #778).** The deny is scoped **per path**, not per git operation, so a `git checkout -B <branch> FETCH_HEAD` that must unlink or rewrite a tracked path under a denied directory — the skills corpus, `.claude/skills/`, is the measured case — is refused for each such path while the branch switch itself is still reported successful: `HEAD` lands on the re-dispatch branch correctly, and the denied working copies silently keep their PRE-checkout content, because git records no conflict for a write it was never permitted to attempt. Both of the checkout's own asserts (`git status --porcelain` empty, `git rev-parse HEAD` matching) can read clean on exactly that state — the mismatch sits only in specific working copies, invisible to either assert. Live occurrence: wave `2026-09-08-blocker-truth-and-call-form`, row 749 iteration 2, disclosed from two directions in one wave — the Worker probed the deny value-free and staged the correct content through the git object store rather than escalate, and the Reviewer, downstream, had to re-run the verify gate against an archive extraction of the committed tip because the corpus-scanning guards read the working tree the Worker's own green run had validated stale.

This is **not** the harness-retry case immediately below, and it is **not** a harness retry OF a re-dispatch either — that second shape (the SAME agent re-run by the harness on an ALREADY-re-dispatched, iteration≥2 worktree) was closed by ruling with no clause of its own (issue #745): nothing about a retry changes which branch or commits are correct, so there was nothing for a clause to add. This clause answers a different question — whether the checkout OPERATION on a re-dispatch iteration itself fully applied — and a fresh iteration≥2 dispatch and a harness-retried one are equally exposed to it. The iteration≥2 workspace-setup brief now carries a clause for exactly that, placed immediately after the checkout's own asserts and therefore before the install step and before any verify command: restore each residual tracked path to the branch tip's content through the FILE-EDITING surface (never by staging through the git object store, which the corpus guards cannot see, because they read the working tree, not the index), re-assert, and stop at `blocked` naming the residual paths and quoting the refusal if it still does not hold. It is a capability refusal like the reset-refused case below — no retry with the sandbox off, no request for one.

**The two rendered briefs stay distinguishable in both directions.** The new clause's headline appears ONLY in the iteration≥2 (re-dispatch) brief — the iteration-1 rendering carries none of it — and, symmetrically, the re-dispatch brief carries none of the iteration-1 inherited-work-in-progress clauses' headlines (§A harness retry reuses the worktree, immediately below). `tools/wave/src/compose-driver.spec.ts` pins both directions over the RENDERED text, the same way it already does for the iteration-1 clauses.
