...
**What reaches the default branch is decided at landing (ADR-0053).** Under `landing.commitMessage` `pr` (the default, and what an absent key means) the landed commit is the PR's title and body, on any commit count, and the Worker's commit messages never reach the default branch; under `host` the repository's squash setting decides. The Worker brief says so beside its title rule.
