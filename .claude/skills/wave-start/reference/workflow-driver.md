...
4. **Capture the printed \`.url\` as your prUrl — and STOP if that capture comes back empty** (wave-shared Convention 12). An empty capture is not "no PR"; it is "the command that should have produced one did not run", and the two are indistinguishable from the empty string alone. Reporting an empty/absent \`prUrl\` on a \`done\` outcome is the live failure this guard exists to stop: the Reviewer skips its PR-body check ("PR is not yet opened"), the Coordinator's terminator reads it as "no PR exists" and opens a duplicate, and the spine records nothing. **Issue the capture and the guard below as TWO SEPARATE Bash calls, never one** — a capture fused onto the guard is refused for the ordinary fusion reason (wave-shared Convention 13). The guard below uses \`if\`, never \`case\`: a \`case\`/\`esac\` guard of this identical shape is refused by the worktree-isolation check **even issued as its own call with nothing fused onto it** — live-reproduced repeatedly and cataloged in wave-shared Convention 13's Catalog section ("jq-piped capture with a case guard") — so splitting alone does not fix it; the guard must avoid \`case\`/\`esac\` entirely, which the \`if\` form below does (verified in the same dispatch that found the \`case\` form refused):
   \`\`\`bash
   # call 1
