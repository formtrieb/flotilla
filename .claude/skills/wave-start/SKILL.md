...
0a. **Disclosure capture (ADR-0027) — the capture half, source-neutral, for EVERY tuple.** Before branching on outcome/verdict, read every disclosure the tuple carries at this, the moment of maximum context: the Worker's `judgmentCalls` (mirrored in `reviewerFocusItems`), the Reviewer's `verdict.reviewerFocusItems`, and anything the Coordinator itself noticed while routing — a Convention 9 wiring gap, a Convention 10 runtime residue, a Convention 11 unfalsified check, or a same-shaped finding of your own. Write each one into the spine now, never deferred to memory: `{{wave-cli}} spine add-disclosure <spine> <id> --iter <iter> --source <worker|reviewer|coordinator> --text "<gap>"` — the verb is source-neutral, all three land identically, and prints the created ref (`<row-id>.<ordinal>`) after the flush. Dispositions like `resolved-in-slice` and `scope-extension` happen naturally right here whenever you already know the exit at routing time (the wiring gap IS the scope extension you're about to grant) — `{{wave-cli}} spine set-disposition <spine> <ref> <disposition>` on that same ref. Everything else starts, and stays, `open` — it rides forward to step 9's report and to `wave-close`'s own fail-closed archive gate (its own slice, ADR-0027), which blocks Archive until every disclosure this wave captured carries a disposition other than `open`. Worked invocation: [reference/start-mechanics.md](reference/start-mechanics.md) §7.0a.
...
3. **Apply the `outcome` (WAL — spine first, then rung):**
   - `{ type: 'transition', nextState: 'approved' }` → run the **terminator**: first render the verdict — `{{wave-cli}} render-verdict <verdictsDir> <id> --anchor <anchorSha>` (`wave-shared` "the reviewer-verdict render") prints the `## Reviewer verdict` section off the MAX-iter valid verdict sidecar, so a changes-requested → re-dispatch cycle's render always reflects the LATEST iteration — then open the PR through the engine — `{{wave-cli}} host-pr create --branch <branch> --title <title> --body <body>` (`approved → pr-created`), **never `gh pr create`**. It is find-before-create idempotent (the Worker that reported an open `prUrl` already opened it; the terminator's create re-pins the same PR — no duplicate). The `--body` carries the rendered verdict section above the **store-kind close phrase** (`wave-shared` Convention 4 — read `store.kind` off `wave.config.json`: `github` → `Closes #<N>`, `linear` → `Fixes <TEAM-NN>`, e.g. `Fixes EX-16`; never hardcode `Closes #N`), and that close phrase is the only tracker id the title/body may name (mention discipline). Then `spine set-row-state <id> pr-created` + `spine set-row-pr <id> <PR-URL>` (the `.url` from the create JSON, or the Worker's reported `prUrl`), then `issue-store transition <id> in-review`.
   - `{ type: 'transition', nextState: 're-dispatched' }` → the **cap=1 re-dispatch**: `spine set-row-state <id> re-dispatched` first, then `spine set-row-iter <id> 2` alongside it (bumps the Plan-Table `Iter` cell + re-renders the sidecar-link cell to the `<id>-2` reports/verdicts paths — observability-only, the reconciler still reads the max-iter sidecar off disk per ADR-0024). **BEFORE the iteration-2 dispatch fires, tear down the row's iteration-1 worktree** — `{{wave-cli}} worktree-cleanup --branches <wave-branch>` (the scoped `--branches` form) — its stale `git worktree` registration otherwise still holds the branch and blocks the iteration-2 checkout (live: W26-F1, the iteration-2 Worker had to unregister it by hand). Then re-dispatch the same Worker at iteration 2 with the Verdict's `changes-requested` items (or the missing context) appended to the brief; its workspace setup is a **tracking-free checkout** of the now-free branch (fetch + `checkout -B <branch> FETCH_HEAD`, or the explicit iteration-1 head SHA — never a tracking form, which writes upstream-tracking into the shared `.git/config` and is sandbox-write-denied for a worktree-isolated agent; full rationale in `reference/workflow-driver.md` §Re-dispatch). The cap of 1 is enforced **by `transition()` itself** — a 2nd `changes-requested` or `needs-context` returns a `stop`, never another re-dispatch.
