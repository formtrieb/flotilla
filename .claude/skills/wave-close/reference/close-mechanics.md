...
## `--expect-head` resolution (ADR-0055)

What lands is the reviewed commit. Every `host-pr arm` and `host-pr merge` that lands a wave row passes `--expect-head <sha>`: the commit `refs/review/<id>` points at, read with `git rev-parse --verify --quiet refs/review/<id>` **before phase 3**, whose `--orphans` sweep removes a terminal wave's own review refs. A PR whose head differs is `refused`, its reason naming both commits: re-dispatch the row, never push to its branch. When the ref is missing, land without the flag and say in the report that the head check abstained for that row (ADR-0052). A Coordinator-direct PR that is not a wave row has no reviewed head and takes no flag.
...
| `{{wave-cli}} issue-store close <id> <prUrl> [--acked 0,2,3]` | the done-reconcile: records the closing PR + cosmetic AC tick from `--acked` (source it from `verdict-acked`, above — never hand-parse a verdict). Idempotent no-op-or-reconcile; on a `states.doneState` store with no integration it forces the mapped done-state transition + a loud advisory (FOR-13). Then probes `read-closing` and prints the resulting `ClosingState` JSON — and, whenever that probe still reads `open` (the satisfied-but-not-by-PR case, e.g. an issue a release resolved), **also** writes an unmistakable `STILL OPEN:` line to stderr naming the id and the recorded `prUrl` (#399) — exit stays `0` either way (ADR-0035 additive: no existing key or exit-code meaning changed). The existing `IssueStore.close()` verb — never re-implemented; no new close verb was added for the satisfied-but-not-by-PR case — the loud line is the cue that the issue stays open until the native close happens, so close it by hand in the tracker instead. |
...
| `{{wave-cli}} merge-order --spine <wave-file>` | `{ "algorithmic": [{ issueId, nn, fileCount, branch, title?, prUrl? }, …], "override": [{ …same shape }, …]\|null, "hasOverride": boolean, "reason": "…" }` — each entry is an OBJECT, never a bare branch string; the phase-4 advisory print reads its `branch` field. |
| `{{wave-cli}} worktree-cleanup [--dry-run] --spine <wave-file> --orphans [--detached]` | `{ "removed": [], "skipped": [], "errors": [], "branchesDeleted": [], "branchHygieneSkipped": [], "orphans": {...}, "detached"?: {...}, "probes": {...}, "worktreeCount": {...}, "unaccounted": {...} }` — always pass `--orphans`; a `0/0/0` `removed/skipped/errors` triple is not evidence of "nothing to do" on its own, check `branchesDeleted`/`orphans`/`probes`/`unaccounted` in the same payload (close-review finding). `detached` is present only with `--detached`; `probes` (the stamped-probe sweep, issue #961) and `unaccounted` (issue #557) are present on every call regardless of flags. With `--dry-run --orphans`, the JSON also carries `orphanBranches: { toDelete, branchHygieneSkipped }` (issue #148) — see phase 3 for the one residual preview/execute asymmetry. |
...
| `{{wave-cli}} host-pr arm --branch <b> [--remote <url>] [--method <squash\|merge\|rebase>] --commit-message <pr\|host> [--expect-head <sha>] [--delete-branch]` | `--auto` landing (ADR-0023): `{ ok, verb:"arm", host, branch, method, commitMessage, outcome:"armed"\|"merged"\|"already-merged"\|"refused"\|"no-pr", prNumber?, prUrl?, reason, branchDeletion?, landingMessage? }`. `--commit-message` per [its resolution](#--commit-message-resolution-adr-0053), `--expect-head` per [its](#--expect-head-resolution-adr-0055). Decides per PR: checks pending → enable auto-merge (GraphQL); already clean → direct merge (REST). Idempotent. Detect-host-routed; **no `--config`** (talks to the code host, not the tracker). `arm` threads the same `--delete-branch` flag as `merge` (consumer KW-F6): an immediate `merged` outcome deletes the head branch synchronously and reports it under `branchDeletion:{ branch, deleted, error? }`; a deferred `armed` outcome has no synchronous merge moment to delete from, so the deferral is recorded in `reason` instead and no `branchDeletion` key is present. |
...
| `{{wave-cli}} host-pr merge --branch <b> [--method …] --commit-message <pr\|host> [--expect-head <sha>] [--delete-branch]` | merge now, no arm intent (caller already decided). Idempotent. Same shape as `arm`, and the same `--commit-message` and `--expect-head` resolutions, plus — with `--delete-branch` (consumer KW-F6) — it deletes the PR's **remote** head branch through the host API after a successful merge and reports the outcome under `branchDeletion:{ branch, deleted, error? }`. A failed delete is a reported degradation (`deleted:false`), **never** a merge failure (exit stays 0). `arm` threads the identical flag (row above) — its immediate-merge outcome deletes the same way; only its deferred `armed` outcome cannot delete synchronously. |
| `{{wave-cli}} host-pr preflight [--remote <url>]` | code-host posture probe for the `--auto` confirm (ADR-0023 amendment): `{ ok, verb:"preflight", host, checks:[{name,status,detail}] }` for `pr-merge-token` / `allow-auto-merge` / `required-checks`, plus ONE host-specific create-verb check fourth: `create-credentials` on `bitbucket`, `pr-create-token` on `github`. Both are advisory-never-fail, and neither is covered by `pr-merge-token` — `create-credentials` grades whether `BITBUCKET_EMAIL` is set (the landing verbs this skill calls authenticate with Bearer and stay unaffected either way), and `pr-create-token` probes whether the GitHub token can actually open a PR, which the repository-role read behind `pr-merge-token` does not establish on a fine-grained token. **Store-blind** — detect-host-routed, **no `--config`**, **no `--branch`** (required checks read against the default branch) — so it answers on **every** store kind, unlike the store-preflight it replaced here. `status` may be `pass`/`fail`/`advisory`/`unknown`; only `fail` blocks. |
...
| `{{wave-cli}} spine set-disposition <wave-file> <disclosure-ref> <disposition>` | dispositions exactly one open entry. `<disposition>` must be one of `resolved-in-slice \| scope-extension \| filed:<id> \| dropped:<reason> \| upstream:<ref>` — anything else, including `open` (the capture default, not a decision), is refused loud with **nothing written**. |
...
| `close` | closing facts recorded (done-reconcile / FOR-13 fallback), then the probed `ClosingState` printed on stdout — `0` even when that probe still reads `open` (a `STILL OPEN:` line on stderr names it, #399; a still-open issue after `close` is a documented outcome, never a domain failure) | issue not found (store threw) | usage (missing `<id>`/`<prUrl>`) |
...
| `spine add-disclosure` | captured at `open`, flushed, the minted ref printed on stdout AFTER the flush (never a ref that did not land) | unknown `<row-id>`, out-of-vocabulary `--source`, empty `--text`, or `--wave-scoped` on a spine whose Plan-Table already took the `wave` id — nothing written | usage: a missing `--source`/`--text`, a non-positive-integer `--iter`, or `--wave-scoped` mixed with a `<row-id>`/`--iter` |
...
#     disposition each: spine set-disposition <wave-file> <ref> <resolved-in-slice|scope-extension|filed:<id>|dropped:<reason>|upstream:<ref>>
...
- `merged` — PR merged; **derive `--acked` via `verdict-acked --verdicts-dir <verdictsDir> --id <id>` (FOR-17), then land it `done` via `issue-store close <id> <prUrl> --acked <indexes>`** (the done-reconcile). On a native-integration tracker the row's `done` also derives from the merged PR's store-kind close phrase (`wave-shared` Convention 4), so `close` is an idempotent reconcile that records the closing facts + the cosmetic AC tick; then clear any stale flag.
...
## Coordination misfires — field purpose (#506)

The closing report's `coordination misfires: N` line (the skill body owns the reporting rule — one-line cause per misfire, an explicit `0` when clean) is a measurement, not a fix. A single wave's count is nearly nothing on its own; what the field is *for* is the series it forms once waves accumulate: **repeated same-shape misfires — the same guessed flag, the same config-miss, the same assumed shape — are the occurrence series the promotion doctrine (ADR-0034) keys on.** One occurrence of a misfire shape is a feeling ("waves feel like they cost more retries lately"); a second live occurrence of that *same* shape is the proof that prose does not hold it, and that recurrence — never this slice — is what licenses spending on structure (a validated flag, a fail-loud config check, a typed return) instead of another retro sentence. A class that stays silently costly forever, because no one counted it, never reaches that trigger at all — which is the gap this line closes: a silently-costly class reaching recurrence becomes a promotion candidate instead of staying a feeling.
...
**A row satisfied by something other than a PR carrying its own close phrase (#399) — a release, a hand action — is NOT this reconcile's job, and `close` gained no new verb for it.** `close` now probes and reports the native end-state loudly (above) so that case is never a silent exit `0`, but the disposition is unchanged: the issue stays open until the native close happens, and a human finishes it by hand in the tracker — decided in the grill that filed #399. Do not reach for a new close verb when you see `STILL OPEN:` — close the issue in the tracker.
