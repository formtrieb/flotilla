...
- **`merged` → land the row `done`** via the **done-reconcile**: derive `--acked` (above), then `{{wave-cli}} issue-store close <id> <prUrl> --acked <indexes>` (the `prUrl` is `readClosing`'s). This is the operational trigger that reaches `done` for a merged row — the wire the live gate found missing (F1) — and, with `--acked` wired, also the checklist tick the live gate found dead (FOR-17): the issue's AC checklist now reads as done for exactly what the Reviewer verified, partial/not-met ACs stay unticked. `close` is **idempotent no-op-or-reconcile**: on a native-integration tracker the merged PR already flipped the coarse projection, so `close` only records the closing PR + the cosmetic AC tick; a re-entrant wave-close re-run never double-posts (re-deriving `--acked` and re-ticking the same indexes is itself idempotent). Do **not** re-implement close — it is the existing `IssueStore.close()` verb. Then clear any stale flag: `{{wave-cli}} issue-store clear-flag <id>`.
  ...
  **Read what `close` itself prints (#399).** Immediately after recording the facts, `close` re-probes and prints the resulting `ClosingState` on stdout — and if that probe STILL reads `open`, it ALSO writes an unmistakable `STILL OPEN:` line to stderr. That is the rare-but-real gap where the merge evidence you just acted on did not translate into a native tracker close (a PR whose close phrase never linked, or a row satisfied by something other than a PR at all — RELEASING.md step 7's release-resolved issue is the worked example). `close` still exits `0` either way — recording the facts always succeeds — but exit `0` here is not "landed `done`" when this line fires. There is no new close verb for this case: follow the documented operator procedure (RELEASING.md step 7) to finish the native close by hand, the same direction decided in the grill that filed #399.
...
- **Reading `close`'s exit `0` as "landed `done`" without reading what it printed (#399).** `close` now re-probes and prints the `ClosingState` after recording the facts; if that probe still reads `open`, it also writes a `STILL OPEN:` line to stderr. Exit `0` means the facts were recorded, not that the issue natively closed — do not skip past the printed state on the assumption that a nonzero exit is the only failure shape here. When it fires, there is no new close verb to reach for: follow the documented operator procedure (RELEASING.md step 7).
