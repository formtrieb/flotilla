...
**The archive move below is also what makes an OLD wave's composed driver sweepable — read `orphans.drivers` beside `orphans.scratch` (issue #748).** `.flotilla/tmp/` holds two populations, not one: the Scribe payload FILES above, and the per-wave `<slug>/` DIRECTORY `wave-start` composes its Workflow driver into. Phase 3 sweeps the directory on one of two routes — every row of the `--spine` spine terminal, or **its spine already sitting in `.flotilla/waves/_archive/`, which is exactly where the move at the bottom of this file puts it**. So the ordering is deliberate and worth knowing rather than rediscovering: at THIS close, the wave being closed is swept by the terminality route (phase 3 runs before the move); at every LATER close, any wave whose spine reached the archive is swept by the archived route. A `<slug>/` directory reported `unknown-wave` is the signal that neither route can see its spine — usually because it was archived somewhere the cleanup run does not look; check `orphans.drivers.wavesDir` in the same output against the destination this file's move actually writes to. Same reporting stance as the payloads: name the count in the close summary, never hold the wave open for it.
...
- **Archiving before terminal.** The terminality gate must hold: every row in `TERMINAL_ROW_STATES`, the engine's exported partition of the row-state vocabulary — `pr-created`/`approved`/`failed`/`abandoned`/`parked`. A row still `dispatched`/`reviewing` means the wave isn't done.
