...
Read [`../wave-shared/SKILL.md`](../wave-shared/SKILL.md) and every file under [`../wave-shared/reference/`](../wave-shared/reference/) first, resolved against this skill's own base directory — no skill invocation, no namespace to guess (ADR-0040) — it owns the auth-preflight / atomic-spine conventions this skill obeys.
...
- **Self-repair hazard — every engine verb this skill calls predates the rows now closing.** `issue-store read-closing`, `merge-order`, `worktree-cleanup`, the `host-pr` routing verbs (`arm`/`merge`/`status`), and `issue-store close` all run from the **local checkout** — and that checkout sits at an **Anchor**: the code as it stood before these rows' own PRs landed, whether that is the one Anchor a wave that lands only at close was dispatched against, or the latest of several a wave that re-anchors between rounds has already pulled forward. Either way it predates *this* close's own merges. A wave whose own rows change any of that machinery cannot reconcile against its own fix until the merge is pulled locally. Phase 4a below gives the mechanical detection step and the required `merge → pull → reconcile` order (W4-F1 / W5-F3).
...
**The terminal states are `TERMINAL_ROW_STATES`, the engine's own partition of the row-state vocabulary** (exported from the package root beside `ROW_STATES`): `pr-created`, `approved`, `failed`, `abandoned`, `parked`. That constant is the source of every list of the five in this skill — here, in [phase 1](reference/phase-1-load-gate.md), in [phase 3](reference/phase-3-worktree-cleanup.md) and in [phase 6](reference/phase-6-archive.md) — and it is the same set the `worktree-cleanup --orphans` terminal-wave verdict and the resume reconciliation branch on. Read it there rather than re-deriving it from the bullets below; the bullets say what each state *means* to this gate, not what the set *is*.
