...
> **The first live `goal` cut that draws a dependency on an initiative-bound goal ran on 2026-08-16, and it disproved both pinned values.** `ProjectRelationCreateInput`'s `type`, `anchorType`, and `relatedAnchorType` fields are `String` in Linear's schema, not enums, so the schema only ever pinned their SHAPE — the confirmation this note used to be pending was reading their VALUES back from a live write against a live Linear workspace, and that read-back has now happened. Neither value this store currently sends held: `PROJECT_BLOCKS_RELATION_TYPE` (`"blocks"`) must be `"dependency"`, and `PROJECT_RELATION_ANCHOR_TYPE` (`"project"`) must be one of `start`/`end`/`milestone` — a whole-project anchor is not among the values Linear accepts. The correction to the two constants in `tools/wave/src/adapters/linear/linear-api.ts` is tracked on its own ticket, outside this documentation pass, and had not landed as of this writing: **the read-back is done, the repair is not.** Until the repair lands, a native project-relation write under an initiative binding still sends the disproven values and Linear still rejects them — `createGoalMember` still surfaces that rejection loudly (a GraphQL error reported as a typed failure naming the minted member, never a silent drop), so nothing is silently lost, but the write itself does not yet succeed. Flip the read-stamp docblock on `PROJECT_BLOCKS_RELATION_TYPE` from disproven to corrected once the repair lands.
