...
>
> **Pending first live run.** The two constants a native project-relation write pins (`type: "blocks"`, `anchorType`/`relatedAnchorType: "project"`) are read from Linear's published schema, not yet confirmed against a live write — `anchorType` is the weaker of the two, an inference from the field's own prose rather than a quoted example. The first cut that draws a dependency on an initiative-bound goal is that confirmation: if the write is rejected, the engine names the minted member so nothing is silently lost; if it succeeds, treat the pinned values as live-verified from then on. Full detail: [reference/goal-mechanics.md](reference/goal-mechanics.md).
