...
- `state: 'open'` → still in review: no action; the row keeps its `in-review` rung. **No-integration workspace with `states.doneState` (FOR-13):** the probe can never report `merged` — it stays `open` even after the PR merged during the outage. When the merge is confirmed out-of-band, run the SAME `{{wave-cli}} issue-store close <id> <prUrl>` to land it via the FOR-13 fallback (mapped done-state transition + loud advisory). `close` is idempotent, so this is safe on a re-run.
