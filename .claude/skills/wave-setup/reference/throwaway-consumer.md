...
| 2 — harness settings | | "writes the permission allowlist a wave needs to run unattended" | only the Bash `cp` is sandbox-refused, not both tools by design — the edit tool is permission-gated only; the skill staged the file, the operator landed it per step 9's two-path form; the Echo-Guard copy needed the sandbox override | README step 2, ONBOARDING, SKILL.md step 9 and the scaffold section now describe the hand-off as the documented path |
