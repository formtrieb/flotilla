...
**What the checks mean on `bitbucket`** (ADR-0023's 2026-08-10 amendment): `allow-auto-merge` is always `advisory`, never a `fail`, because Bitbucket Cloud exposes **no arming primitive** — there is no setting to tick, so a permanently-red gate would be noise. `required-checks` reads Bitbucket's `require_passing_builds_to_merge` branch restriction, which states a **count** rather than named contexts, so it reports the minimum and names nothing. `pr-merge-token` reads a **user-scoped** permission endpoint, which an access token has no user context for — a `pass` there means "no evidence of a read-only credential", and its `detail` says so; the merge write remains the ground truth.
