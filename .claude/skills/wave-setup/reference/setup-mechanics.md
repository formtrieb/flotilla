...
```json
{
  ...
  "permissions": {
    "allow": [
      ...
      "Bash(npx tsx tools/wave/src/resume-cli.ts:*)",
      "Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/resume-cli.ts:*)",
      "Bash(npx tsx tools/wave/src/spine-cli.ts:*)",
      "Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/spine-cli.ts:*)",
...
- **Retained direct-entrypoint aliases — `resume-cli.ts` and `spine-cli.ts` (issue #269).** `cli.ts`'s own router keeps three pre-unification entrypoints alive as documented ALIASES to the identical runner (its own in-source NOTE: they "survive as documented ALIASES only because live skill call-sites still spell them that way") — `resume-cli.ts`, `cli-store.ts` (its `preflight` op — see the store-preflight section above), and `spine-cli.ts`. Two of the three have live consuming call-sites that spell them as the SEPARATE entrypoint rather than the `{{wave-cli}} <sub>` form: `wave-resume`'s SKILL.md invokes the reconciler as `npx tsx tools/wave/src/resume-cli.ts …` under its own heading "Call the reconciler (its own entrypoint — NOT a `cli.ts` subcommand)"; `wave-close`'s close-mechanics.md documents `npx tsx tools/wave/src/spine-cli.ts <op> …` as "the direct spine entry the top-level `spine` relay wraps." Same fallback logic as the trailing two `cli.ts` entries just above: scaffold both binary styles (npx-free local binary + `npx tsx`) for a consumer that vendors `tools/wave` locally, omit both entirely for a fresh plugin consumer with no vendored copy. `cli-store.ts`'s own alias form has no corresponding allowlist entry on this repo's own live tracked settings as of this reconciliation — nothing to scaffold there yet; noted so a future author doesn't mistake the gap for a deliberate omission.
...
A prior slice traced exactly one live-only entry — `Bash(mkdir -p:*)`, resolved into the "Sidecar/archive directory creation" bullet above — and stopped there, on the Reviewer's own endorsement of that narrow reading. This section closes the rest: every entry this repo's own tracked `.claude/settings.json` carries beyond what the scaffold documents, classified in full, so a future author never has to re-derive which extras are legitimate and which are debris.

**Scope.** This reconciliation covers `permissions.allow` only — the array the scaffold JSON block above reproduces. `permissions.deny` and the `env` block are **already at full parity** between the live file and the scaffold above (verified entry-for-entry: the same 18 `deny` lines, including the ADR-0029 credential-lookup additions; the same three `env` keys) — nothing to reconcile there. `enabledPlugins` (the source-form self-disable, root CLAUDE.md `### Distribution`, ADR-0032) and `hooks` (the echo-guard `PreToolUse` hook, wave-shared Convention 8) are different top-level `.claude/settings.json` keys, already documented at their own home (root CLAUDE.md; wave-shared's Convention 8 reference) — outside "the allowlist" this issue names, so not re-litigated here.
...
The live file's `npx @formtrieb/flotilla-engine` pair, the dotted vendored `cli.ts` pair (plus their `resume-cli.ts`/`spine-cli.ts` companions folded in above), `git merge-tree`, the five workspace-mechanic git verbs, `git push origin wave/:*`, `git worktree add:*`, the three verify-gate commands, and `mkdir -p:*` all already match an entry this scaffold documents — no delta on any of those. The genuine delta is the following 20 entries.

| Live entry | Classification | Disposition |
|---|---|---|
| `Bash(npx tsx tools/wave/src/resume-cli.ts:*)` | scaffold-worthy | Folded into the scaffold JSON + bullet above (this PR). |
| `Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/resume-cli.ts:*)` | scaffold-worthy | Folded into the scaffold JSON + bullet above (this PR). |
| `Bash(npx tsx tools/wave/src/spine-cli.ts:*)` | scaffold-worthy | Folded into the scaffold JSON + bullet above (this PR). |
| `Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/spine-cli.ts:*)` | scaffold-worthy | Folded into the scaffold JSON + bullet above (this PR). |
...
2. Leave the 4 scaffold-worthy `resume-cli.ts` / `spine-cli.ts` entries as-is — they are already correct live; this reconciliation only changed the scaffold to document what was already there.
