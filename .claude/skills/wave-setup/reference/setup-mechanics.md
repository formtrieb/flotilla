...
**A second rule on `install`, with a second reason: its DIRECTORY ARGUMENT must be repo-relative too.** Write `npm ci --prefix tools/wave`, never `npm ci --prefix /Users/…/tools/wave`. This one is not about the allowlist — it is about the package manager. Measured on npm 11.17.0: `npm ci --prefix <dir>` exits `EUSAGE` with `Missing: <basename>@<version> from lock file`, naming a package that appears in neither the manifest nor the lockfile, whenever that prefix path resolves **through a symlink**. Positive and negative control on the *same* directory — `/tmp/claude-501/…/tools/wave` fails, `/private/tmp/claude-501/…/tools/wave` succeeds (macOS resolves `/tmp` → `/private/tmp`). npm's own machinery, probed directly, prints the mechanism: the ideal tree's root comes back as a `Link` at location `"../../../../../../../tmp/claude-501/…/tools/wave"` while the lockfile's root sits at `""`, so `validateLockfile` finds no entry for it; and the name reported for that root is `wave` — the prefix directory's basename, via `@npmcli/name-from-folder` — while `pkg.name` on the very same node reads `@formtrieb/flotilla-engine`. A **repo-relative** prefix cannot reach that state, because npm resolves it against the process's working directory, which the OS always reports physically. Two corollaries worth writing down, both measured: nothing rescues the symlinked form by flag (`--install-links`, `--install-strategy=nested`, `--legacy-peer-deps`, `--omit=dev` all still fail), and `npm install` "survives" it only by never running the lockfile-vs-manifest comparison at all — so swapping the verb trades the guarantee away instead of satisfying it. The engine does **not** currently refuse an absolute path in an argument position (`normalizeEngineInstall` checks index 0 of the whole binding), so this rule is on the author, not on the validator.
