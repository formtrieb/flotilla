...
`config validate` proves the JSON parses; **the store-preflight proves the live TRACKER preconditions hold**, probed *for real* through the engine's existing API seam — no separate integration script. The code-host posture (merge token, allow-auto-merge, required-checks) is a **separate owner** — see the host-preflight section below (ADR-0023 amendment: one fact, one owner).
...
| `gitAutomation` (a FIELD beside `checks` — advisory, never moves `ok` or the exit code) | absent | **probed** — the team's PR-automation rules (`Team.gitAutomationStates`); team-default rules graded: `draft`/`start` → `states.inFlight`, `review`/`mergeable` → `states.inReview`; `merge` reported only. `aligned` / `misaligned` (names event, found state, expected state) / `abstain` (a graded event with no team-default rule, or the read unavailable). A null-state rule reads "no action", never a mismatch. Branch-scoped rules are listed, never graded — they and the abstaining case stay a human check (SKILL.md item 6) | absent |
...
```json
{
  ...
  "gitAutomation": { "status": "abstain", "undecided": ["draft"], "rules": [ … ], "detail": "Cannot decide: …" }
}
```
...
| `create-credentials` | **Bitbucket-only** — omitted (`null`) from the checks array on `github` and every other host, because it grades an AMBIENT variable GitHub has no equivalent of (GitHub's create credential is a token pairing with no second input that could be missing). Not because GitHub's create right is already graded elsewhere: it is not, and `pr-create-token` below is where it now is. On `bitbucket`: `pass` when `BITBUCKET_EMAIL` is set, **`advisory`** (never `fail`) when it is absent or an empty string — the landing verbs (`arm`/`merge`/`status`/`preflight`) authenticate with Bearer and stay unaffected either way; only `host-pr create` needs the Basic-auth pair this check grades, and its `detail` names that consequence plus the fix. |
| `pr-create-token` | **GitHub-only** — the fourth check there, and `create-credentials`' opposite number: it PROBES the create right rather than grading a variable, because on this host the precondition is a property of the token as GitHub sees it and nothing local can. Two read-only requests with `create`'s own credential (the identity read, then the open-PR list read `create` performs before it opens one). `pass` when both answer; **`advisory`** (never `fail`) on a denied identity read, a denied list read, or a transport failure; `unknown` when no credential resolved to probe with. **`pr-merge-token` does not cover this** — it reads the repository ROLE (`GET /repos/{o}/{r}` → push/maintain/admin), and a fine-grained token can hold the role while being scoped away from `Pull requests` entirely: measured on both rows of a consumer wave (2026-09-04) that passed `pr-merge-token` and then 403'd on `host-pr create`, at every row's termination step, after the work was done. |
...
| `GET /repos/{o}/{r}/commits/{ref}/check-runs` | `host-pr arm` check-attach evidence, half one (Checks-API apps, GitHub Actions included) | `repo` | Checks: Read — off "List check runs for a Git reference", [rest/checks/runs](https://docs.github.com/en/rest/checks/runs), re-read 2026-09-21 |
| `GET /repos/{o}/{r}/commits/{ref}/status` | `host-pr arm` check-attach evidence, half two (external CI on legacy commit statuses) | `repo` (that page names no per-endpoint classic scope; `repo` is what covers a private repo's commit reads) | Commit statuses: Read — off "Get the combined status for a specific reference", [rest/commits/statuses](https://docs.github.com/en/rest/commits/statuses), re-read 2026-09-21 |
...
**`repo` (classic) covers every row above in one scope.** A fine-grained PAT does not bundle the same way — GitHub splits `repo` into separate repository permissions, so a fine-grained setup needs **`Issues`, `Pull requests`, and `Contents`, each Read and write**, plus **`Checks: Read` and `Commit statuses: Read`** for the two check-attach rows, named individually. Granting only the one the arm mutation needs (`Pull requests`) is the exact way a fine-grained token passes `store-preflight` (which never labels an issue) and then fails the first `addLabel`/`removeLabel` call mid-wave.

**The two check-attach rows fail QUIETLY.** A token without them still lands: the reports read throws, the arm treats a failed read as *no evidence* (never as "nothing has attached"), and the PR merges as it would have. What is lost is the verification, so the arm's `reason` says the merge was `NOT verified against the required-check names` and names which endpoint refused, with its status. Read that clause before concluding the host cannot report required checks — `host-pr preflight` reads them off a *different* endpoint and will name them in the same session.
...
Same discipline as the GitHub table above: neither preflight substitutes for checking scope up front. `store-preflight` never touches the code host at all, and `host-pr preflight` grades only what its four checks can see — a token can clear both and still fail `host-pr create`, gated on `BITBUCKET_EMAIL` rather than on a scope. Two independent things to get right: the **scope**, and — per [SKILL.md's Bitbucket token section](../SKILL.md#bitbucket-token--the-two-flavors-and-which-one-host-pr-create-needs) — **which flavor** of token is being scoped (an account API token and a workspace/repository access token are not interchangeable; the table below applies to the account-API-token flavor, the one that can open a PR at all).

**The documented minimal set is four scopes, not two.** Every landing verb (`status`/`preflight`/`arm`/`merge`) and, paired with `BITBUCKET_EMAIL`, `create` too, issue at least one pull-request call, and Atlassian's own API spec gates every one on the separate `pullrequest`/`pullrequest:write` pair — never the repository pair alone. Table below: the granular API-token scope-picker form. Spec source and re-measurement date: [evidence/bitbucket-token-scope-measurements.md](../evidence/bitbucket-token-scope-measurements.md#the-documented-minimal-set--spec-provenance).
...
**Predicted-vs-live status of the four-scope set.** A live round-trip on exactly these four scopes remains the open item. Measurement provenance: [evidence/bitbucket-token-scope-measurements.md](../evidence/bitbucket-token-scope-measurements.md#predicted-vs-live-status-of-the-four-scope-set).

**`read:pipeline:bitbucket` — recommended.** Without it, `allow-auto-merge` and `required-checks` report `unknown` (never `fail`), and `--auto` cannot tell a clean row from a red-build one. Granting it moves both to `advisory` with real content, making `wave-close --auto`'s refuse logic for failing builds reachable. Live-verification detail: [evidence/bitbucket-token-scope-measurements.md](../evidence/bitbucket-token-scope-measurements.md#readpipelinebitbucket--the-live-verification-detail).

**`admin:repository:bitbucket` does not subsume the four scopes above and is not in the minimal set** — grant the four regardless; open collision and why: [evidence/bitbucket-token-scope-measurements.md](../evidence/bitbucket-token-scope-measurements.md#adminrepositorybitbucket--the-open-collision-not-a-settled-fact).
...
**A second rule on `install`, with a second reason: its DIRECTORY ARGUMENT must be repo-relative too.** Write `npm ci --prefix tools/wave`, never `npm ci --prefix /Users/…/tools/wave`. This one is not about the allowlist — it is about the package manager: `npm ci --prefix <dir>` exits `EUSAGE` with `Missing: <basename>@<version> from lock file`, naming a package that appears in neither the manifest nor the lockfile, whenever that prefix path resolves **through a symlink**. A **repo-relative** prefix cannot reach that state, because npm resolves it against the process's working directory, which the OS always reports physically. Nothing rescues the symlinked form by flag (`--install-links`, `--install-strategy=nested`, `--legacy-peer-deps`, `--omit=dev` all still fail), and `npm install` "survives" it only by never running the lockfile-vs-manifest comparison at all — so swapping the verb trades the guarantee away instead of satisfying it. The engine does **not** currently refuse an absolute path in an argument position (`normalizeEngineInstall` checks index 0 of the whole binding), so this rule is on the author, not on the validator. Mechanism, npm version, and the measured positive/negative control: [evidence/engine-cli-symlink-mechanism.md](../evidence/engine-cli-symlink-mechanism.md).
...
`commitMessage`, optional: exactly `"pr"` or `"host"`; absent means `"pr"`. `pr` sends the PR's own title (plus the host's number suffix) and body as the landed commit, on any commit count, so the Worker's commit messages never reach the default branch — except under a merge queue, which composes its own commit, and `--method rebase`, which lands the commits themselves with no single message to shape. `host` sends nothing, so the repository's own squash setting composes the commit; it is for a history a tool reads (semantic-release, commitlint, a changelog generator). The landing verbs never read this key; wave-close passes it as `--commit-message` on every `host-pr arm` and `merge`. `config validate` refuses any other value at exit 1, naming `landing.commitMessage` and both values, echoes a declared one on its `ok:` line (`landing.commitMessage: host`), and only warns on a misspelled key inside `landing`.
...
Confirmed by execution (2026-07-31, issue #372): with the discovery root pinned, this form resolved **vitest 4.1.8**, ran **62 test files, 2789 passing tests**, and the tsc form exited **0**. The plausible-looking bare form (`npx vitest run` / `npx tsc --noEmit`) is what hand-deriving the spelling from the description tends to compose instead — and it resolves to **nothing** on this repo's own shape, reaching the npm registry mid-dispatch rather than the local binary.

**Why the profile itself has to name the pinned form, not a prose warning beside it.** The identical gap — an unpinned or under-specified verify spelling standing in for the pinned one — recurred in three distinct shapes on three separate occasions, and a warning survived none of them, including the third occurrence, where the agent had just read it. The fix has to remove the bare spelling from reach entirely, which is exactly what naming the pinned, discovery-rooted form directly inside `commands` does. Full measured comparison (the bare-form JSON, exact test counts) and the three-occurrence recurrence arc: [evidence/verify-command-measurement.md](../evidence/verify-command-measurement.md) — three further spellings of this same pair, audited from the allowlist-reconciliation angle (which spellings are *used* vs. *allowlisted*, rather than which spelling *resolves*), are in ["2026-07-31 pass"](#2026-07-31-pass--the-used-but-absent-direction-issue-291) below.
...
Before writing this config, walk through the SKILL.md "Linear operational preconditions" checklist with the consumer (GitHub integration installed, the team's PR-automation aligned to `states.inFlight`/`states.inReview`, `Fixes <TEAM-NN>` PR-body convention, PR-route discipline, the `states.unclaimTarget`-vs-`states.queued` team convention) — only the integration and, advisorily, the team-default PR-automation rules are engine-checkable, so `config validate` passing does not mean these hold.
...
> **How the file actually lands — the two-path hand-off (revised 2026-09-21, issue #734).** Two mechanisms, not one: the sandbox denies a Bash `cp` here structurally, by design; the edit tool is NOT structurally refused, only subject to the ordinary permission gate, so it is mode-dependent — live occurrence, issue #734: a Bash `cp` here was classifier-denied in the same session the edit tool applied the identical change with no complaint.
>
> **Path 1, tried first:** stage the complete, valid JSON below in the session scratchpad, then write it straight to `.claude/settings.json` with the edit tool — if the gate permits it, the file lands with no hand-off at all. **Path 2, only if declined:** hand the operator one copy command to run from their own prompt — `cp <staged path> .claude/settings.json`. **Never lead it with the CLI's `!` shell-mode prefix**: that is CLI-only, and a host without it (the VS Code extension, confirmed live) routes the line to the model as plain text, so the copy silently never happens.
>
> **Either path, the read-back is mandatory:** read the file back and confirm it matches the staged copy byte for byte, then re-run the value-free checks (`credential-probe --all`, `host-pr preflight`) so the new `env` block is proven live — the read-back is the only thing that caught the VS Code no-op live. The guard-hook copies (below) land the same two-path way.
...
- **The read of a file at a given commit — the refused-reset remedy's own source (issue #744).** `Bash(git show:*)` — `wave-start`'s iteration-1 workspace setup (`workflow-driver.md`, step 2) prescribes `git show <anchorSha>:<path>` as the source a Worker restores from when its `git reset --hard` is refused: the harness write-deny is scoped **per tool surface**, so the shell reset can be refused where the FILE-EDITING tool writing that same path is not, and the remedy is to read the anchor's copy and write it back through the surface that works. Without this entry the brief could only *hedge* the read ("where your permission surface carries that read"), which is a recovery one permission prompt away from a stalled row — the exact AFK failure NF-F1 measured. **Ruled and measured 2026-09-16 (Operator):** the command form is `git show <anchorSha>:<path>`, it exits **0** under a dispatched agent's sandbox, and it is granted as the **prefix match** `Bash(git show:*)`. The width is safe in a way `git clean`'s is not (next bullet but one): `git show` writes nothing — no prefix of it can reach a sibling worktree's files — so there is no confinement question to answer.
...
- **Probe-worktree creation.** `Bash(git worktree add:*)` — the Reviewer's "probe license" (`wave-reviewer`'s SKILL.md: the Reviewer stays code-read-only but MAY create a scratch worktree to independently exercise an outcome the diff alone can't prove). Narrower than a blanket `git worktree:*`: only the creation subcommand is allowlisted, not `remove`/`prune`/`list`. That absence is **deliberate, not an omission (decided in the doctrine-budget grill, 2026-08-09)**: an allowlist pattern cannot scope `git worktree remove` to the worktree the agent itself created — any grant would reach every sibling's live worktree, trading the fan-out's parallel-safety for agent discipline. **What the host-side lifecycle guarantees in exchange is that the Coordinator reaps the probe by its stamp — never that the agent does** ([ADR-0042](../../../../docs/adr/0042-the-sweep-owes-accounting-never-removal.md) Amendment 2026-09-23, decisions 13–14, replacing the "named, not reaped" guarantee this bullet used to state). The probe must live outside the repository — an in-repo checkout aborts on the harness write-deny at any depth — so it sits in no containment root; instead the path handed to `git worktree add` must itself end in the stamp `flotilla-probe-<wave-slug>-<row-id>-i<iteration>` (never a parent directory), and the stamp admits it to `worktree-cleanup` wherever it sits. The Coordinator runs that sweep (`--probes-only`) right after routing each round's verdicts, removing every probe whose row is not running at the probe's iteration, and wave-close phase 3 collects what routing missed — rights the Coordinator already holds. Only an UNSTAMPED leftover still lands in the run's **`unaccounted`** list for the **Operator** (issue #557). Do not add a `remove`/`prune` entry when a dispatched agent asks for one; the refusal is the design, and the Coordinator's stamp sweep is the guarantee behind it.

  **`git clean` is the SECOND deliberate absence from this scaffold, for the same confinement reason (Operator ruling, 2026-09-16 — issue #744).** The untracked-file sweep is genuinely the right shell command for the leftover an aborted or partial `git reset --hard` leaves behind, and `wave-start`'s iteration-1 setup names it by name — as a **dead end**, not as a prescription. The reason is `git worktree remove`'s, verbatim: a permission entry is a **prefix match over a command string**, and no prefix of `git clean` can be confined to the worktree the agent itself is running in — any grant would reach every sibling's live checkout, and an untracked-file sweep there destroys work no commit is holding. So the brief accounts for the untracked leftover instead of sweeping it (a path the Worker's own write will rewrite is discarded when that write lands; a path left standing is ADOPTED and owes a recorded review), and this bullet's ruling is what the brief cites. Do not add a `git clean` entry when a dispatched agent asks for one — and note the contrast with `Bash(git show:*)` two bullets above, which is granted at full prefix width precisely because a read has no confinement question to answer.
...
- **The remote-tip confirmation before a sibling fetch (issue #979).** `Bash(git ls-remote origin:*)` — the same Check 5 has the Reviewer read `git ls-remote origin refs/heads/<branch>` before it trusts any fetched sibling ref, and again for the default branch the landed-sibling check targets. Read-only, so it is granted at full prefix width with no confinement question. Without it a run with nobody watching stalls on a permission prompt mid-review.
...
### Guard hooks scaffold — `hooks.PreToolUse` block + script copies (Echo-Guard + Convention-12 guard; Convention 8 stage 2 — gate MET)

Two guard hooks ship in the engine package — the Echo-Guard (Convention 8) and the Convention-12 guard (the unquoted-parameter-expansion blocker) — and this scaffold writes **both the same way, unconditionally**: the same two artifacts per hook, the same tracked destination directory, the same single `hooks.PreToolUse` block. Convention 8's stage-2 gate (wave-shared's Convention 8 reference) is **met**: wave `2026-07-29-conventions-wiring` ran 12 agents across 374 tool calls with zero Echo-Guard rejections and zero false positives. Each ships **unconditionally, for every consumer**, on the same footing as the `permissions.deny` anchors above — never one hook scaffolded and the other left to a by-hand copy (issue #762).

**Two artifacts per hook, both required — a hooks block pointing at a script the consumer does not have is worse than no block at all:**

1. **The script copies, into a TRACKED path — the same two-path landing as the settings file above, not a third mechanism** (this destination directory sits under the identical structural Bash-`cp` denial). `node_modules` is gitignored, so a dispatched Worker/Reviewer worktree never sees a script left there. **Path 1:** read each source script and write it straight to its destination with the edit tool. **Path 2, only if declined:** hand the operator the commands below to run from their own prompt — never the leading-`!` shortcut:

   ```bash
   mkdir -p .claude/hooks
   cp node_modules/@formtrieb/flotilla-engine/hooks/echo-guard.cjs .claude/hooks/echo-guard.cjs
   cp node_modules/@formtrieb/flotilla-engine/hooks/conv12-guard.cjs .claude/hooks/conv12-guard.cjs
   git add .claude/hooks/echo-guard.cjs .claude/hooks/conv12-guard.cjs
   ```

   **Either path, read both files back and confirm they match their source scripts byte for byte before continuing.**

   `node_modules/@formtrieb/flotilla-engine/hooks/` is the sibling directory the pinned package already ships alongside the binary `engine.cli` resolves to — both hooks live there side by side.

2. **The `hooks.PreToolUse` block**, merged into the consumer's tracked `.claude/settings.json` as a sibling of `env` and `permissions` — the same file the scaffold above already writes. Both hooks are entries in the SAME `"Bash"`-matcher `hooks` array, not two separate `PreToolUse` blocks:
       ...
       "PreToolUse": [
         {
           ...
           "hooks": [
             ...
             {
               "type": "command",
               "command": "node \"$CLAUDE_PROJECT_DIR/.claude/hooks/conv12-guard.cjs\""
             }
           ]
         }
       ]
...
**flotilla's own repo is the one documented exception, not a second scaffold form** — the same shape as every other exception this file records for its own dogfood config. Its `engine.cli` binds to the vendored `tools/wave/` tree, already tracked, so step 1's copies are a no-op here; this repo's own tracked `.claude/settings.json` names `tools/wave/hooks/echo-guard.cjs` and `tools/wave/hooks/conv12-guard.cjs` directly, in that same two-entry `hooks` array, instead of the per-consumer destinations from step 1.

**Read this as landed, not as the vector being closed.** The gate being met rolls out the hooks block and the script copies everywhere; it says nothing about what either guard itself is — each stays, unchanged, a **text matcher over the command string**, not an anchor (own header + wave-shared's Convention 8/12 references). Two vectors stay deliberately outside the Echo-Guard's own and are never "handled" by this rollout: reading a gitignored settings/secrets file, and the *direct* invocation of a configured `<VAR>_CMD` lookup command — both owned by the `permissions.deny` entries this same scaffold already writes ([Credential lookup-command scaffold](#credential-lookup-command-scaffold-adr-0029) below).

**Landing-order note (issue #215), for both hooks.** Step 1 assumes the published `@formtrieb/flotilla-engine` package's installed tarball ships its `hooks/` directory — true of the package's own `files` manifest (`tools/wave/package.json`) at the time of writing, but a fact about the *published* artifact's currency on a given consumer, not about this doc. On a consumer whose installed package predates that fix, the `cp` source is simply absent and the copy fails loudly (a missing-file error), never silently — re-run `npm install` against a current version first.

**Present-but-stale is the second failure mode, and it is silent — pin the version floor.** A package can ship `hooks/echo-guard.cjs` and still carry an old copy: the `cp` succeeds, nothing errors, and the consumer runs a weaker guard than the one this doc describes (live occurrence: `0.1.0-beta.1` shipped a guard predating the family-3 quote-nesting carve-outs). The guard copy therefore has a **version floor: require `@formtrieb/flotilla-engine@>=1.0.0`** before scaffolding; on an older install, upgrade first — the loud missing-file case above is the only failure the `cp` itself will ever report. The destinations step 1's commands write to — `.claude/hooks/echo-guard.cjs` and `.claude/hooks/conv12-guard.cjs` — are the **fixed convention** for the copies, applied identically to both, and other docs cite them by those paths.
...
**Re-copy on every engine version bump** — same present-but-stale failure mode, recurring at every bump, not only at first scaffold; re-run step 1's two `cp` lines for both hooks and commit. The exact consumer-facing step lives in [docs/ONBOARDING.md](../../../../docs/ONBOARDING.md); the release-note-carries-it duty for flotilla's own maintainers is separate and does not belong in this skill.

### Trackability verification (issue #494)

All three artifacts this scaffold writes — the tracked `.claude/settings.json` above and the tracked guard-hook copies at `.claude/hooks/echo-guard.cjs` and `.claude/hooks/conv12-guard.cjs` (step 1's two `cp` destinations in the guard-hooks subsection above) — land inside `.claude/`, and a consumer's pre-existing `.gitignore` is exactly as likely to already exclude that whole directory as any other path in the repo. Writing any of them then succeeds on disk with no error; `git add .claude` stages nothing and still exits 0; `git status` reports a clean tree. Nothing in that sequence is loud — a `wave-setup` run can report success with the scaffold silently absent from every future worktree, and every dispatched agent then runs with no allowlist, no env block, no guard hooks (the NF-F1 stall class, arrived at silently — see the "Beyond the engine CLI" note above). **Run this verification immediately after [Procedure step 9](../SKILL.md#procedure) writes the scaffold, before reporting success — never take a clean `git status` as proof.**

```bash
git check-ignore .claude/settings.json
git check-ignore .claude/hooks/echo-guard.cjs
git check-ignore .claude/hooks/conv12-guard.cjs
```
...
- **Exit 1 (no match) on all three** — trackable. Continue to the cross-check below.
- **Exit 0 (a match) on any** — **STOP.** Do not report success. The consumer's `.gitignore` excludes a path this scaffold depends on; fix the `.gitignore`, never the scaffold — see the narrowing recipe below.
...
`.claude/*` excludes every direct child of `.claude/` — files and subdirectories alike — without excluding `.claude/` itself, so git still descends into it. `!.claude/settings.json` then re-includes that one file; `!.claude/hooks/` re-includes the `hooks/` directory as a unit, and because nothing excludes *its* contents, both `echo-guard.cjs` and `conv12-guard.cjs` come back in with it. `.claude/settings.local.json` and any other file directly under `.claude/` this recipe does not name stay ignored, exactly as intended — neither appears in a negation.

#### The `git add --dry-run` cross-check

Confirm the narrowing actually resolved the way the three `git check-ignore` calls above imply, from the staging side:

```bash
git add --dry-run .claude
```

It must list exactly the three scaffolded paths — `.claude/settings.json`, `.claude/hooks/echo-guard.cjs`, and `.claude/hooks/conv12-guard.cjs` — and nothing else new under `.claude/` (an already-tracked file re-listing is expected; a *new* file beyond the three scaffolded ones is not, and is its own signal to re-check the `.gitignore` edit). A shorter or empty listing means the narrowing recipe did not take.

### Scaffold-vs-live allowlist reconciliation (issue #269)

**The rule: every entry this repo's own tracked `permissions.allow` carries must trace to the generic scaffold above, this repo's documented vendored-form exception, or the dogfood-only table below.** Why: an allowlist entry nobody can re-derive a reason for is exactly the debris a future author either trusts wrongly or deletes wrongly. Scope: `permissions.allow` only — `env` and `permissions.deny` are covered separately (the deny-parity divergence is recorded in the 2026-07-31 pass below); `enabledPlugins`/`hooks` are different top-level keys documented at their own home.

The original pass diffed the live file against a scaffold that then named six engine-CLI forms (since collapsed to one, ADR-0032) and found 20 live-only entries: 4 became this repo's documented vendored exception, 16 were stale pre-router (`src/*-cli.ts`) entrypoints handed to an operator PR for removal. Issue #345 performed that removal — none of the 16 remain live today.

**Going forward, a new `permissions.allow` entry must arrive with either a scaffold citation** (added to the JSON block above in the same change, with its rationale stated inline) **or an explicit dogfood-only note** (naming, here or in the landing PR, why this repo's own dogfood setup needs it and no generic consumer would) — never land uncited.
...
**The #269 reconciliation above runs in exactly one direction: live entry → is it cited?** It never asked what the file *lacked* — a command a dispatched agent runs a hundred times with no allowlist entry prints green under that direction alone (the Convention 9 wiring-gap shape, applied to the allowlist itself). This pass reads the converse: is every *declared* verify command actually on the permission surface?

**Method.** Every `Bash` tool call in this repo's session transcripts since 2026-07-26 was extracted from the on-disk JSONL — coordinator sessions *and* the per-worktree AFK Worker/Reviewer transcripts (their own `~/.claude/projects/<worktree-path>/` roots, missed entirely by a scan of the coordinator's directory alone). 694 transcripts, 644 worker-side, 12,337 Bash calls, each matched against the live `permissions.allow`.
...
**Why this never showed up as a permission prompt, and why that is not reassurance.** Zero prompts across all 12,337 calls — the gap was masked end-to-end by two allowlists outside the dispatch contract, the operator's own user-level `~/.claude/settings.json` (`Bash(npm *)`, `Bash(npx *)`, …) and the gitignored `.claude/settings.local.json`. Neither reaches a fresh consumer, and the gitignored one does not even reach this repo's own Worker worktrees — a masked-but-incomplete tracked file passes a "no new prompt" live gate for the wrong reason.
...
**Exact-form matching against a redirected command — still open.** The exact-form entries (`Bash(npm test)`, `Bash(npm run typecheck)`, `Bash(./tools/wave/node_modules/.bin/tsc -p tools/wave --noEmit)`, `Bash(npx tsc --noEmit)`) assume the permission matcher splits on pipe/redirection before matching, rather than requiring the whole invoked string to equal the entry byte-for-byte — every measured call in practice carries a suffix (`npm test 2>&1 | tail -40`). A live measurement ran the exact-form `tsc` entry both bare and with a redirect+pipe suffix from a worktree-isolated dispatch and found no observable difference — not a clean falsification, since that dispatch class showed zero friction on every command it ran that session, entry or no entry, so a masking layer independent of the tracked allowlist cannot be ruled out. **What would actually falsify this**: an environment provably running with *only* the tracked `.claude/settings.json` as its permission source — the piped branch should prompt there if the assumption is wrong and the bare branch should not; if a future wave ever sees that asymmetry, the fix is a `:*` suffix on the entry that prompted.

**Deny-parity divergence.** The live `permissions.deny` no longer matches the scaffold exactly: `Read(.claude/settings.local.json)` / `Read(**/.claude/settings.local.json)` were removed by the operator after ADR-0029 moved every credential into the keychain lookup path, leaving that file holding no secret. The scaffold above deliberately keeps **both** lines regardless — a generic consumer's `settings.local.json` may still hold live credentials, and the vector Convention 8 catalogues is universal. The two `Read` lines are a **pair by necessity**, root form plus `**/` form — `**/` alone does not match the repo-root path (observed directly). The `.env` anchors above are the same pair for the same reason; do not simplify either pair to its `**/` half.
...
**Grounding the go/no-go decision beyond the baseline probe alone.** A discriminating baseline probe does not, by itself, prove that a *declared, tracked* `sandbox` entry would widen a dispatched agent's own baseline — a dispatched agent cannot write `.claude/settings.json` and observe the effect on itself. Issue #716 grounded the go/no-go decision in Claude Code's own published settings documentation and a structural analogy to the already-live `permissions` block instead of assuming either way: full citation and reasoning in [evidence/sandbox-block-scope-grounding.md](../evidence/sandbox-block-scope-grounding.md).
...
The SKILL.md [Credentials](../SKILL.md#credentials) section owns the **judgment** (the two first-class paths, the live-gate vs. "ACL bind" framing, the AFK-incompatibility of per-invocation-confirmation resolvers, the direnv+keychain half-measure); this is the concrete scaffold. Like the AFK harness config above, it lands in the consumer repo's **tracked** `.claude/settings.json` — the same two-path landing (above), the same file, not a second one — because a dispatched Worker/Reviewer worktree carries tracked files only, and the `<VAR>_CMD` entry has to reach that worktree for the wave's own dispatched agents to resolve the credential, not only the operator's interactive session.
