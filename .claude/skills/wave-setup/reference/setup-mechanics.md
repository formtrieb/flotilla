...
It must list exactly the two scaffolded paths — `.claude/settings.json` and `.claude/hooks/echo-guard.cjs` — and nothing else new under `.claude/` (an already-tracked file re-listing is expected; a *new* file beyond the two scaffolded ones is not, and is its own signal to re-check the `.gitignore` edit). A shorter or empty listing means the narrowing recipe did not take.
