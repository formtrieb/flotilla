...
**flotilla's own repo is the one documented exception — not a second scaffold form.** This repo binds `engine.cli` to the vendored value, and its OWN tracked allowlist reflects that single substitution, not an addition: `./tools/wave/node_modules/.bin/tsx tools/wave/src/cli.ts` (+ its env-prefixed twin) in place of the two lines above. Two more vendored-path entries currently survive alongside it here — `tools/wave/src/resume-cli.ts` and `tools/wave/src/spine-cli.ts` (+ their env-prefixed twins) — because `wave-resume` and `wave-close`'s own resolution blocks still hardcode those direct-entrypoint paths rather than the unified `{{wave-cli}} resume` / `{{wave-cli}} spine` subcommand form that `cli.ts`'s router already supports. That is a live, accurate description of this repo's own tracked file today, not a second consumer-facing entry to reproduce: a fresh consumer scaffold never carries any of these four — a Node consumer's `engine.cli` already resolves `resume`/`spine` through the same single binary as everything else, once the resolution blocks that still spell them separately are brought in line (tracked as a follow-up to this slice, outside its declared scope).
- **Why this repo's own exception still names four entries, not two — `resume-cli.ts` and `spine-cli.ts` (issue #269).** `cli.ts`'s own router keeps three pre-unification entrypoints alive as documented ALIASES to the identical runner (its own in-source NOTE: they "survive as documented ALIASES only because live skill call-sites still spell them that way") — `resume-cli.ts`, `cli-store.ts` (its `preflight` op — see the store-preflight section above), and `spine-cli.ts`. Two of the three have live consuming call-sites in THIS repo's own skill docs that still spell them as the SEPARATE entrypoint rather than the `{{wave-cli}} <sub>` form: `wave-resume`'s SKILL.md invokes the reconciler as `npx tsx tools/wave/src/resume-cli.ts …` under its own heading "Call the reconciler (its own entrypoint — NOT a `cli.ts` subcommand)"; `wave-close`'s close-mechanics.md documents `npx tsx tools/wave/src/spine-cli.ts <op> …` as "the direct spine entry the top-level `spine` relay wraps." Those two literal invocations are why this repo's own allowlist still needs the extra pair beyond the vendored `cli.ts` form — not because a fresh consumer ever needs them (see the paragraph above). `cli-store.ts`'s own alias form has no corresponding allowlist entry on this repo's own live tracked settings as of this reconciliation — nothing to scaffold there yet; noted so a future author doesn't mistake the gap for a deliberate omission.
...
> **This reconciliation predates the ADR-0032 scaffold rewrite above** — it diffed the live file against a scaffold that still named six engine-CLI forms (the published package, the local `tsx` binary, and the `resume-cli.ts`/`spine-cli.ts` aliases, each with and without the proxy prefix). The generic scaffold now names exactly one form, so the four `resume-cli.ts`/`spine-cli.ts` rows below have moved — they still describe live, correct entries in THIS repo's own tracked file, but as this repo's documented exception (above), not as generic-scaffold entries. Everything else this table found and dispositioned (the 16-entry stale-pre-router-path removal) is unaffected and already actioned. A fresh reconciliation pass against the new single-form baseline is a follow-up outside this slice's declared scope.
...
|---|---|---|
| `Bash(npx tsx tools/wave/src/resume-cli.ts:*)` | scaffold-worthy (at the time) | Folded into the scaffold JSON + bullet at the time; now this repo's documented exception (see the ADR-0032 rewrite note above), not the generic block — still live and correct, reclassified only. |
| `Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/resume-cli.ts:*)` | scaffold-worthy (at the time) | Same reclassification. |
| `Bash(npx tsx tools/wave/src/spine-cli.ts:*)` | scaffold-worthy (at the time) | Same reclassification. |
| `Bash(./tools/wave/node_modules/.bin/tsx tools/wave/src/spine-cli.ts:*)` | scaffold-worthy (at the time) | Same reclassification. |
