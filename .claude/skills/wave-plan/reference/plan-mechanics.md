...
- `blockedByCycles` (ADR-0054) — the **cycle report**: each entry names every issue on one dependency cycle among the candidates and the claimed set, listed along the edges (each blocked by the next, the last by the first); the CLI also prints each as a stderr `cycle:` line. No row on a cycle can ever dispatch, so surface it to the Operator — never propose a cycle's rows as runnable. `issue-store block` refuses to close a cycle, so one found here was drawn past it, usually by hand in the tracker; the fix is `issue-store unblock <id> --by <blocker-id>` on the wrong edge.
