...
Every command needs the store config: run from a dir containing `wave.config.json`, or append `--config <path>`. The store (`markdown`, `github`, or `linear`) is selected there — a PRD becomes a `prd`-labelled issue on GitHub, a `prd.md` on a markdown store, or a native Document in the configured project or team on Linear.

## Commands

The Document facet lives under the `issue-store` verb, like every other store operation.

| Call | Purpose |
|---|---|
| `issue-store publishDocument --input <f.json>` | publish the PRD → prints the opaque id |
| `issue-store readDocument <id>` | verify the round-trip → `{ id, title, body }` |
| any command, no args | usage |

## `PublishDocumentInput`

A PRD is published through the store's **Document facet** — `issue-store publishDocument`, *not* `issue-store create`. The input carries only a title, a filing hint, and the PRD sections — **no** Risk/Worker/Files/AC/Header-Block (those belong to the slices `to-issues` derives later):
...
```bash
{{wave-cli}} issue-store publishDocument --input <prd.json>   # prints the opaque PRD id
```
...
```bash
{{wave-cli}} issue-store readDocument <id>
```
