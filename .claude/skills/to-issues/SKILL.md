...
> **The list is read twice, and the two readings pull apart.** To the conflict-map, Files is a *conflict boundary* — wider is safer, per the rule above. To the Worker executing the row, the same list is a *design instruction* — it reads as "stay strictly inside this," because that boundary-reading is exactly what keeps the conflict-map trustworthy at runtime (the Reviewer's own diff-against-declared-globs check enforces it). Both readings are correct in isolation; they collide only when the list names exactly one new file per concern that doesn't exist yet, and the Worker then puts new code where the list allows rather than where the repo's own conventions say it belongs. Seen live twice in one wave (docs/retros/2026-07-27-plugin-consumer-w1.md, DA-F9): a view model landed inside its view file, and an authorization type inside an unrelated refresher — both against convention, both honestly disclosed under `judgmentCalls`, and the design cost was paid anyway.
>
> **So for files that don't exist yet, widen by concern, not by count.** Name the file **you want to come into existence** for each distinct concern — the shape the work should take — not the minimum set that happens to cover the diff. A row that will produce a view model and the view that uses it declares **both files**, even though "one new view file" would technically satisfy the conflict-map. "Wider" here means *more files, each with a reason*, never the same work squeezed into fewer places.
...
Run `dor` on each published issue: its **self-content gates** (header-parseable, risk-file-count, AC-coverage) prove the slice is grabbable *now* — on every store, github included. Working-tree gates (e.g. `literal-files-exist`) show `deferred` when run on a bare github id and are re-checked at `wave-create`; only a self-content FAIL blocks (ADR-0014). For a batch that may share files, run `conflict-map` and surface the overlap cells so the user can plan serialized lanes. Commands: [reference/filing-mechanics.md](reference/filing-mechanics.md).

> **The title rule of step 1 is checked right here, by id.** Run by id, `dor` reads the row's own title off the tracker, so the `pr-title-id-independent` line actually answers instead of deferring: it warns when the title leans on an issue number the pull-request title will delete, and prints both titles — the tracker's and the one the pull request would open under — so you read the orphaned sentence rather than imagining it. This is the cheapest moment to re-word it. If a title genuinely cannot avoid the number, **declare the pull-request title instead of deriving one**: pass `--pr-title '<the title the PR should open under>'` and the line passes, because nothing is derived any more. It never blocks a row — the advisory warns and the verdict is unchanged — so treat a warning as a question to answer now, not a gate to argue with. And read `deferred` on that line for what it says: nobody could give the check a title, *not* that the title is clean.
