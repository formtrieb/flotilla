...
**2. Heredoc spec append** — narrower than "fusion": refused only when a heredoc redirects straight to a file and its body contains `{`/`}`, not otherwise, and no verified working recipe for the JSON-patch use case this shape names ([Convention 6](convention-06-sanctioned-amend-path.md)'s `issue-store amend <id> --patch <json-file>`) was found in this dispatch — the code pair and full reasoning are in the evidence sidecar.
