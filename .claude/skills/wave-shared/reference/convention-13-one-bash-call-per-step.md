...
**Live-reproduced twice, in two separate dispatches, both `isolation: 'worktree'`:**

- *2026-07-30, this convention's own dispatch:* `cd <worktree>/tools/wave` returned success, and the very next call's `npm ci` failed with npm's own usage error because it ran at the worktree root — `pwd` in the following call printed the worktree root, not `tools/wave`.
- *2026-07-31, issue #251's dispatch:* the same pair, minimal form. Call 1 `cd <worktree>/tools/wave` — exit 0, no output. Call 2 `pwd` — printed the **worktree root**. Nothing carried; nothing failed loudly either, which is exactly why the assumption survives unnoticed until something depends on it.
