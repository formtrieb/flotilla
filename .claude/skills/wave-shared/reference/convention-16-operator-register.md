...
- **A byte-identical short clause in every SKILL.md body.** It carries the register rule, the direct address, the form switch, and a pointer back here. It is planted from one source constant and a drift guard deep-equals every copy against it, so no copy can diverge.
...
- **[ADR-0039](../../../../docs/adr/0039-user-directed-output-holds-the-operator-register.md)** — the decision this convention carries: the audiences, the five clauses, the form switch, the two structural guards it decides on (the clause drift guard over these skill bodies, which ships with this convention, and a descriptions guard over the frontmatter, which lands in a later rollout row), and the rejected options — strict-everywhere with no switch, pointer-compromise everywhere, the full convention text copied into each skill. The decision record lives with the others under `docs/adr/`.
- **[ADR-0040](../../../../docs/adr/0040-cross-skill-loading-is-a-sibling-path-read.md)** — the sibling-path read this file's pointer rides. Cross-skill loading is a file read against the loading skill's own base directory; this convention's long form is its first non-execution consumer.
