...
POSIX `sh` and `bash` **word-split** an unquoted parameter expansion; **zsh does not** (`SH_WORD_SPLIT` is off by default). So `$CLI verb` under zsh looks for a command whose name is literally `npx some-package`, finds none, and exits **127**. The same line is correct in one shell and inert in the other, which is exactly why it keeps being written by people who have written it successfully before — the reflex transfers, the shell does not.
