...
**Still open, and deliberately outside this slice's declared files** — named so the next reader finds them rather than rediscovering them:

- `wave-shared/reference/convention-04-store-kind-close-phrase.md`'s `VERDICT_SECTION=$({{wave-cli}} render-verdict …)`. In class: empty means `render-verdict` did not run, and the PR body would then be composed *without* the rendered verdict while still reading as complete. (`start-mechanics.md`'s copy is reshaped — it now sits inside step 7c's single call.)
- `wave-plan/reference/plan-mechanics.md`'s and `wave-create/reference/create-mechanics.md`'s `T=$(mktemp -d)`. In class: an empty `T` turns every `> "$T/x.json"` into a write at the filesystem root. Note the sharper reading this repair gives them: those files use `$T` across *several* documented steps, so the capture is not merely unguarded — it is one Bash call away from being unset.

That is the whole census: every other `VAR=$(…)` across `.claude/skills/` is either inside a single self-contained call, one of the deliberate non-guards below, or `start-mechanics`' `WSTATE`, whose `$([ … ] && echo a || echo b)` form cannot produce an empty value. Re-run that census before adding a row to either half of this ledger.
