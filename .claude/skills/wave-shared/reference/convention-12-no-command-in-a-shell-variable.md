...
**Enforced by:** hook — `tools/wave/hooks/conv12-guard.cjs` (half one, the unquoted expansion); reference doc — this file (half two, the empty-capture discipline — a prose rung, because no text matcher sees a caller's next step).
...
The promotion history — the doctrine-budget grill (ADR-0034), the occurrence count that earned the hook, and why the prose below remains load-bearing regardless of it — moved to evidence (history: `../evidence/convention-12-no-command-in-a-shell-variable.md`, read via the sibling-path read when actually wanted, ADR-0040).

Why the guard's own scope reaches wider than this convention's name, the parsing-complexity reason it stays a quote scanner rather than a shell parser, and the 2026-08-13 occurrence that first surprised this repo with a blocked value expansion — moved to evidence (history: `../evidence/convention-12-no-command-in-a-shell-variable.md`, read via the sibling-path read when actually wanted, ADR-0040).
...
No canonical body remains to copy — a shell function is session state, and the session it assumed does not exist — and the full retirement note, including the resurrection warning, moved to evidence (history: `../evidence/convention-12-no-command-in-a-shell-variable.md`, read via the sibling-path read when actually wanted, ADR-0040).
...
The six-form/four-control measurement behind this rule, the retracted status-code identification, who is exposed and who is not, and the three-item remedy list all moved to evidence (history: `../evidence/convention-12-no-command-in-a-shell-variable.md`, read via the sibling-path read when actually wanted, ADR-0040).

#### The tension with this convention's own workaround — named, because it is real

This convention's own guard hook sends an operator into exactly the nested context this rule warns about — a real tension, not an oversight — and the reconciliation (why the remedy still holds, and the one command family that remains unsafe there) is in evidence (history: `../evidence/convention-12-no-command-in-a-shell-variable.md`, read via the sibling-path read when actually wanted, ADR-0040).
