...
**Enforced by:** brief prose — `tools/wave/driver/wave-start-inflight.js` (the policy clause a Worker is handed; runtime-resource knowledge stays out of the node-pure engine by design).
...
The engine stays untouched here: runtime-resource knowledge (docker, ports) deliberately does not enter the node-pure core (the two-layer split, CHARTER §4) — a structural cleanup sweep remains a possible future slice only if this convention proves insufficient in practice.
