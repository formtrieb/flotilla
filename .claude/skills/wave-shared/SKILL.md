...
1. **The canonical agent-boundary JSON schemas** — `WORKER_REPORT_SCHEMA` and `REVIEWER_VERDICT_SCHEMA`, inlined verbatim below. A driver script cannot `import` a TS const, so the shipped driver carries its own literals and hands them to `agent({ schema })`; nobody pastes them — `compose-driver` fills that shipped script and the harness runs the file (issue #680). They are **copies** of `tools/wave/src/worker-report-schema.ts` / `reviewer-verdict-schema.ts` and the drift-guard spec (`tools/wave/src/skill-schema-drift.spec.ts`) deep-equals them to the exported consts on every run — if you edit a literal here, the spec fails until the source const matches.
...
Each Convention this skill's siblings inherit lives as its own file under [reference/](reference/) — one file per convention, carrying its full `## Convention N — …` heading and body verbatim, including every rule and its one-line why. **Load every file in that directory, not a subset picked by name** — a sibling's `Convention <n>` citation resolves to real prose only once the whole directory has been read, and no single file stands in for the rest. This loader deliberately does not enumerate what's in there — only the number allocated most recently (below): a new convention is a new file dropped into `reference/`, with zero edits to any *existing* reference file required.

A convention's growing live-occurrence history — the wave-by-wave record backing its why — is **not** part of what this loader pulls. Where a convention has one, it lives one level over in a same-named file under [evidence/](evidence/), a sibling `reference/` never reaches, and the reference/ file carries a one-line pointer to it instead of the narrative itself (ADR-0034's Amendment, 2026-08-13). Read an evidence file only when the provenance behind a rule is actually wanted, via the same sibling-path read a `Convention <n>` citation already resolves through (ADR-0040) — never as part of the standing per-wave load. A convention with no live-occurrence history yet has no evidence file.
...
- **Pasting this file's canonical `WORKER_REPORT_SCHEMA` (with `anyOf`) straight into `agent({ schema })`.** The agent tool rejects a top-level `anyOf`/`oneOf`/`allOf` at the boundary (live: W5-F1). The anyOf-free copy is the one in the shipped driver script, `tools/wave/driver/wave-start-inflight.js`, which `compose-driver` fills and the harness runs — there is no literal to paste from `workflow-driver.md`, only the reasoning about it. On that driver copy the `prUrl`-on-`done` invariant is brief-enforced, not schema-enforced.
...
- **Reaching for raw `gh` to arm, merge, or probe a PR.** Landing and the host merge-probe go through the `host-pr arm | merge | status` verbs (Convention 7, ADR-0023) — `gh` left the landing path entirely (sandbox-denied creds, TLS failing under the sandbox — ADR-0015). These verbs take no `--config` (they talk to the code host, not the tracker).
