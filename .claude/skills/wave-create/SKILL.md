...
- **Using the path-only `conflict-map` CLI to build the spine's Conflict-Map.** The `conflict-map` CLI operates on markdown file paths; it cannot read a bare GitHub id (the M1 target). Use `cross-wave` with the `IssueView[]` arrays, then extract `intraWaveConflicts` from the `CrossWaveResult`.
