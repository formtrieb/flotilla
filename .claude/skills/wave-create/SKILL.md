...
**`verify-profile-coverage` (advisory) — a row matching no verify profile must be *stated*, not silently equivalent to a gated one.** DoR now also intersects each row's declared `Files:` against the consumer's `wave.config.json` verify profiles' `appliesTo` globs — the same set arithmetic the cross-wave conflict check already performs, just against profiles instead of another row's files. `pass` means EVERY declared file cleared at least one profile — the row is fully verify-backed. A `warn` means the row's declared files match **no** configured verify profile: nothing will be compiled or tested for it, and a later Reviewer `approve` on that row is inspection-only, not verify-backed. This is a legitimate outcome, not a defect to fix — some work genuinely has no automated gate — so it never blocks and it never fails. It also never fires spuriously on a consumer with no verify profile configured at all (an empty or absent `verify`), and on a bare id with no repo checkout it `defer`s rather than guessing.

**The PARTIAL case is its own outcome, not a shade of either the pass or the all-uncovered warn above (issue #711).** A row can have *some* declared files covered by a profile and *some* not — the observed-common shape is a feature row spanning both a tested package's sources and untested application targets. Before issue #711 this read as a plain `pass` (matching "at least one" was enough), which made the uncovered half indistinguishable from a fully verify-backed approve. Now it `warn`s, naming the specific uncovered files and the profile(s) that DID match, and stating that an approve is verify-backed only for the covered files — inspection-only for the rest. Treat this exactly like the all-uncovered warn below: it is not a defect in the row (some files legitimately have no gate) and it never blocks — but it must be carried forward, not summarized away.
...
Capture the per-issue results — the full narrative becomes the spine's `## DOR-check` section (the one opaque, skill-side section; the engine never parses it). Any `verify-profile-coverage` warn belongs in this narrative **by name** (row id + the files that matched nothing) — whether it is the all-uncovered warn or the PARTIAL warn (issue #711): for a partial hit, name the row id, the specific uncovered files verbatim, and the profile(s) that DID match, exactly as you would for a fully-uncovered row. And so does any `files-touched-since-tracker-update` warn: row id, the commits the gate named, and the outcome of the re-read you did ("premise re-read against `main`, still holds" / "row amended first" / "dropped"). Do not summarize any of them away as a generic "warnings present" line. `wave-start` re-runs DoR per row at its own dispatch step, so the same warn resurfaces there; keeping it explicit here is what lets it reach the Worker and Reviewer briefs — and, when the Coordinator seeds that row's `reviewerHints` at dispatch, the uncovered files belong there verbatim too, so the Reviewer sees exactly which half of the row it is inspecting unbacked — instead of dead-ending in a narrative nobody reads closely.
...
The result is `CrossWaveResult { parallelSafe, crossWaveConflicts, intraWaveConflicts, intraWaveBlockedByPairs, blockedByCycles, warnings? }`.
...
- **`blockedByCycles` → the cycle report (ADR-0054), a third launch-gate.** Each entry names every issue on one dependency cycle; no row on it can ever dispatch. If non-empty, name each cycle to the Operator and **ask, default abort** — the fix is `issue-store unblock` on the wrong edge, never dropping one row and pretending the other is free.
...
- **Reading a PARTIAL `verify-profile-coverage` warn as if it were a plain `pass` (issue #711).** A row where only some declared files match a profile is not fully verify-backed — it is a distinct outcome from full coverage, and folding it back into "pass, nothing to note" is exactly the bug this gate was fixed to stop. Carry the uncovered files verbatim into the narrative and the row's reviewer hint, the same as you would for an all-uncovered row.
