...
Two states share that same `deferred` status text but are not the same fact, and the narrative you write must not collapse them: **resolvable** — `--config` was passed on the `dor` call above, `wave.config.json` loaded, and its `verify.profiles` (empty or not) were actually weighed against this row's files, so `pass`/`warn` is a real verdict — versus **genuinely absent** — no `--config` reached this call, so `verify` was never even in view and the `defer` says nothing about the row's coverage, only that this invocation didn't check. The DoR step above should land in the resolvable state on every ordinary run; a `defer` here is worth a second look, not a shrug. What it changes is that the gap can no longer pass unnoticed: name the warning row and its unmatched files explicitly when you write the narrative below, so an `approve` later in the wave is a choice the Coordinator/Reviewer made knowingly, not an accident indistinguishable from a fully test-backed row.

Capture the per-issue results — the full narrative becomes the spine's `## DOR-check` section (the one opaque, skill-side section; the engine never parses it). Any `verify-profile-coverage` warn belongs in this narrative **by name** (row id + the files that matched nothing) — do not summarize it away as a generic "warnings present" line. `wave-start` re-runs DoR per row at its own dispatch step, so the same warn resurfaces there; keeping it explicit here is what lets it reach the Worker and Reviewer briefs instead of dead-ending in a narrative nobody reads closely.
