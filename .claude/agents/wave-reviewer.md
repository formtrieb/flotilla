...
- **Reach the code host through the engine, never a raw host CLI** (wave-shared [Convention 7](../skills/wave-shared/reference/convention-07-host-landing-seam.md)). Reads are not the exception: `host-pr status --branch <branch>` is how you learn a PR's state, its url, or that its body carries the close phrase — `gh pr view` is not a shorter way to the same fact, it is a second credential path nothing in this system reviews.
