...
The opt-in `auto` mode keeps this ADR's core property (server-side completion — survives a dead Coordinator) but is refined twice. The shape is **partial-arm**: the per-wave confirm arms only the order-free rows (those in no Conflict-Map pair); overlapping rows keep the recomputed advisory order as the human playbook — not arm-all + fall-behind flags, which would knowingly convert *predicted* overlaps into `needs-attention` noise. And the transport is **the engine host seam, not `gh`**: GraphQL `enablePullRequestAutoMerge` / REST merge under the ambient `GITHUB_TOKEN` (the token the FOR-12 preflight already probes for merge rights) — this ADR's literal `gh pr merge --auto` wording is superseded (gh's sandbox-denied creds + its TLS failing under the sandbox, live-proven P-6/w2-F4 — mechanism unmeasured beyond that; see the 2026-09-16 correction below).

## Correction (2026-09-16 — the keychain/proxy-TLS mechanism was never measured)

The amendment above, in its parenthetical, named the mechanism behind `gh`'s sandbox TLS failure as "keychain/proxy TLS failures". **Withdrawn.** A 2026-09-08 measurement with controls (recorded as [ADR-0015](0015-triage-is-a-tracker-agnostic-triage-facet.md)'s evidence note) ran `curl` and `git` through the identical nested shell form, against the identical host and proxy, in the same session — both passed, which a keychain fault or a proxy interception certificate breaking `gh` would not have survived. The same measurement's sandbox-off control passes on the identical failing `gh` form, which is what names the actual layer: the sandbox, not the keychain and not the proxy. Nothing in this ADR's decision changes — the transport is still the engine host seam, for the reasons that independently hold (sandbox-denied creds, one auditable CLI verb); only the keychain/proxy mechanism claim is retracted.
