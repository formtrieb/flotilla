...
- **2026-09-25 — two exceptions to "under `pr` the PR title and body land", and the refresh's failure edge.** Decision 1 says the verb sends a title and a body on every landing, and that holds. What does not hold everywhere is that they *land*:
  - **A merge queue composes its own commit.** GitHub's GraphQL schema notes on `EnablePullRequestAutoMergeInput` that "when merging with a merge queue any input value for commit headline is ignored", and the same for the commit message and the merge method (docs.github.com/public/fpt/schema.docs.graphql, read 2026-09-25). A repository that lands through a queue keeps the queue's message whatever `arm` froze.
  - **A rebase landing has no single message to shape.** `REBASE` is "Add all commits from the head branch onto the base branch individually" (same schema), so the branch's own commits land, the Worker's commit messages among them, and the title and body shape nothing.
  - Neither is detected at runtime. Every shipped statement of what lands under `pr` now names both: the landing docs, the wave-close landing phase, the `host-pr` help text and the `landing.commitMessage` refusal. `arm`'s `landingMessage` is described as frozen only where the host uses it; under either exception it reports what was handed over, not what lands.
  - **The re-arm refresh's failure edge.** Decision 3 left the refresh mechanism to the implementation, and the GitHub adapter disables then re-enables. If the disable succeeds and the re-enable fails for any reason other than the two typed refusals, the PR is left unarmed. The error now says that auto-merge was disabled, that the PR is unarmed, and that re-running `host-pr arm` restores it. The typed refusals route as before. The refresh itself has not yet been observed live; its failure edge is specified against a fake host.
