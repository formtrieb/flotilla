...
| 5 | **Get issues wave-ready** | `triage` works incoming issues into a ready state; `to-prd`/`to-issues` turn a plan or spec into wave-eligible issues (each carrying a declared file scope, a risk/worker classification, and acceptance criteria) — or hand-author issues in the same shape. |
| 6 | **Run `wave-plan`** | Draws the current wave-eligible candidate set and cross-checks it against anything another wave already has claimed. Read-only and advisory — you pick which ids go into a wave. |
| 7 | **Run `wave-create`** with the chosen ids | Materializes a spine (the durable per-wave orchestration record) and sets the soft `queued` claim on each issue. |
| 8 | **Run `wave-start`** | Every row gets a Worker (worktree-isolated agent) and, once the Worker reports, a universal Reviewer (schema-validated verdict, deterministic routing to approve / request-changes / stop). Ends with every non-held row in-review — **it never merges anything.** |
| 9 | **Review and land the PRs, then run `wave-close`** | Land through your normal code-host flow, or through the engine's `host-pr` verbs where wired. `wave-close` computes the advisory merge order (accounting for declared file overlap between rows), cleans up the agent worktrees, and archives the spine. |
| 10 | **Coordinator died mid-wave? Run `wave-resume`** | Don't restart from scratch. It reconciles state from the spine (the write-ahead-log authority), the live worktrees, and the on-disk sidecars, then re-dispatches only what actually needs it. |
