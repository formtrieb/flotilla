...
- **Environment keys — placement is decided, not a choice.** `GITHUB_TOKEN` for a `github` store and for the engine's PR-landing (`host-pr`) verbs; `LINEAR_API_KEY` for a `linear` store. **Export both as shell environment variables, in your shell profile, before running any wave skill — never in `.claude/settings.json` or `.claude/settings.local.json`.** The reason is structural, not a preference: `wave-start` dispatches every Worker/Reviewer into a worktree that carries **tracked files only**, so the gitignored `.claude/settings.local.json` never reaches it (verified live against a dispatched worktree — see the `wave-setup` skill's Credentials section), and the tracked `.claude/settings.json` is committed to the repo, so a secret placed there ships in the repo's history forever. A shell-profile export is the one placement that reaches every wave skill invocation *and* every dispatched agent, through ordinary process-environment inheritance — no settings-file plumbing involved or possible. Both adapters talk raw HTTP to their own API — there is no `gh`/`git` subprocess dependency for reading or writing issues — and the engine fails loudly and immediately if a required key is missing.
