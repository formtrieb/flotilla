...
| PR **title + body** on `host-pr status` (read-only) | ✓ `title` + `body`, off the payload the state read already fetched — no extra request | ✓ `title` + the body from `description`, falling back to the schema-listed `summary.raw`; same single list response, no extra request. The two spellings are a documented discrepancy, not hedging: Atlassian's own OpenAPI document (read **2026-09-16**) omits `description` from the `pullrequest` schema's `properties` while the create endpoint's prose documents it against that same schema ("Other fields: `description` - a string"), and `summary: {raw, markup, html}` *is* listed, `raw` being "The text as it was typed by a user". The read itself is **spec-proven only**: `status` was live-run on this host 2026-08-17 (row below), but that run predates these two keys, so no live payload has confirmed which spelling a real workspace sends |
