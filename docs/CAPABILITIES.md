...
| PR-automation alignment read at setup (`store-preflight`'s `gitAutomation` reading) | absent — no tracker-side PR automation; the engine writes the `wave/*` labels itself | **advisory** reading, never fails the preflight or moves its exit code: the team's `gitAutomationStates`, team-default rules graded against the configured claim states — `aligned` / `misaligned` / `abstain` (a graded event with no team-default rule, or the read unavailable); branch-scoped rules listed, not graded — [2026-09-25 amendment](adr/0020-linear-claims-live-in-workflow-states-triage-vocabulary-stays-labels.md#amendment-2026-09-25--the-pr-automation-mapping-is-readable-and-store-preflight-reads-it-as-an-advisory-reading-issue-879). Schema read 2026-09-26; spec-proven, `verify`: the first `store-preflight` on a live linear team | absent |
...
| Blocked-by added / removed after filing (`issue-store block` / `unblock`, [ADR-0054](adr/0054-a-dependency-found-after-filing-is-written-one-edge-at-a-time.md)) | header + native dependency mirrored; `unblock` deletes the native edge and exits 1 naming it if it survives — the native delete is spec-proven, not yet live-proven | header + native relation mirrored; `unblock` deletes the relation, same honesty rule — spec-proven, not yet live-proven | header line only (no native edge) |
| Dependency-cycle refusal / report | `block` refuses an edge that closes a cycle; `cross-wave` reports a cycle drawn past it | same | same |
...
| **Expected head** (`arm`/`merge --expect-head <sha>`, [ADR-0055](adr/0055-what-lands-is-the-reviewed-commit.md)) — a PR lands only at the reviewed commit, else `refused` naming both | ✓ pinned **by the host**: the REST merge's `sha` (409 on a mismatch) and `expectedHeadOid` on `enablePullRequestAutoMerge`, behind the verb's own request-free comparison of the head it just read | the verb's own comparison only — Bitbucket's merge has no field to pin a head to (`pullrequest_merge_parameters`: `type`, `message`, `close_source_branch`, `merge_strategy`; read **2026-09-26**). A **check-then-act window**: a push between the verb's status read and the merge request is not seen. The head may arrive abbreviated (`commit.hash` pattern `[0-9a-f]{7,}?`) and is compared as a prefix |
...
| PR **title + body** on `host-pr status` (read-only) | ✓ `title` + `body`, off the payload the state read already fetched — no extra request | ✓ `title` + the body from `description`, falling back to `summary.raw`, then to `rendered.description.raw`; same single list response, no extra request. `description` is primary because it is the field this engine writes itself (`createBody`'s POST body, in `host-pr create`) and reads back itself (`bbRef`, off the very same find-before-create query) — an engine-observed fact, not a vendor-documented response field: Atlassian's own OpenAPI document (read **2026-09-16**) omits `description` from the `pullrequest` schema's `properties` entirely. `summary: {raw, markup, html}` and `rendered: {title, description: {raw, markup, html}}` *are* schema-listed, `raw` in each being "The text as it was typed by a user". The read itself is **spec-proven only**: `status` was live-run on this host 2026-08-17 (row below), but that run predates these three keys, so no live payload has confirmed which spelling a real workspace sends |
| Posture probes (allow-auto-merge, required checks, merge token) | ✓; a visible OFF grades `fail` when required checks are present, `advisory` when none are; `unknown` (below maintain/admin rights) never blocks. A fourth check, `pr-create-token`, probes the CREATE right with two read-only requests — advisory-never-fail, because a land-only consumer is healthy without it | advisory grading only — a visible OFF names a UI affordance for a human, not an engine capability, so there is no misconfiguration to flag ([ADR-0023](adr/0023-landing-is-partial-arm-through-the-engine-host-seam.md) 2026-08-10 amendment) |
...
| Credential shape | `GITHUB_TOKEN`/`_CMD`: classic PAT `repo` scope, or fine-grained Pull requests + Contents (Read and write), plus Checks: Read and Commit statuses: Read for the arm's check-attach evidence. The create right is **not** implied by the merge right on a fine-grained token — the role read behind `pr-merge-token` can pass while `Pull requests` is unscoped (measured on a consumer wave, 2026-09-04), which is why `host-pr preflight` probes it separately as `pr-create-token` | `BITBUCKET_TOKEN`/`_CMD` + `BITBUCKET_EMAIL`: Atlassian API token over Basic auth; **four scopes** — `read`/`write:repository:bitbucket` + `read`/`write:pullrequest:bitbucket` (re-measured 2026-08-15 against Atlassian's OpenAPI spec — the pull-request pair gates every landing call, not the repository pair alone). App passwords stopped working **2026-06-09**, removed **2026-07-28** |
