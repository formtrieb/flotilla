...
- **PR-route by default.** Protected `main`, PR-only landing for *everything* — including orchestration artifacts. This makes the Ur's **§7.1 harness auto-mode FF-classifier gate vanish**: that gate fires only on *direct* default-branch pushes, and with branch protection there is no such path. The spine **rides its own branch** — `spine/<slug>` as designed in ADR-0048; until that is built, whatever branch the Coordinator's checkout is on — and is a branch-local archive; it does **not** need to land on `main` before its archive. The whole "session-end FF-sync ritual" the Ur needs simply does not exist here. *This is the one decision flotilla deliberately makes differently from the Ur.*
- **Merge, terminality & `done` (ADR-0005).** `wave-start` ends the dispatch/review loop at **all rows `in-review`** and proposes an *advisory* initial merge-order (never merges, never `done`). **`wave-close`** is the merge phase: it **recomputes** the order (PRs get revised post-review) and either recommends it (default) or — **opt-in `auto`** — **arms** the order-free rows through the engine host seam (`host-pr arm`; `gh` left the landing path — ADR-0023's partial-arm), letting the host complete server-side (survives a dead Coordinator); the overlapping tail keeps the advisory order as the human playbook, and a PR that cannot arm is reported. **Wave terminality = all rows `in-review`**; the **spine is archived at `wave-close`**, not at `wave-start`. **`done` is derived** (natively closed via `Closes #N`), so `close()` is no-op-or-reconcile.
- **Coordinator = a (mostly-idle) foreground session; Workers = background agents** it spawns into isolated worktrees under the repo's worktree **Containment root** — never a system temp dir (ADR-0042). As long as a STOP needs a live conversation to pause/ask/resume, the Coordinator cannot be a pure background run; the typed Ask/Answer protocol (ADR-0047) removes exactly that need, which is what makes the headless Pulse (ADR-0048) possible.
...
- **Written:** [`0027`](adr/0027-disclosures-are-spine-captured-at-routing-and-dispositioned-before-archive.md) — **Disclosures** (Convention 9/10 gaps, Reviewer and Coordinator finds) are captured in the spine at verdict-routing through paired verbs and must reach a **Disposition** (`resolved-in-slice · scope-extension · filed:<id> · dropped:<reason> · upstream:<ref>`) before `wave-close` archives — existence enforced mechanically, quality left human; amended 2026-07-31 with the triviality default and thematic bundles.
- **Written:** [`0028`](adr/0028-wave-shared-conventions-are-self-registering-reference-files.md) — the `wave-shared` barrel splits into self-registering reference files, dissolving the seven-row clique on one SKILL.md; the driver template deliberately stays one file.
- **Written:** [`0029`](adr/0029-credentials-resolve-through-a-per-project-lookup-command.md) — credentials resolve through a per-project **Lookup-Command** (`<VAR>_CMD` in the tracked settings env block) — configured wins and fails loud, the ambient variable stays first-class for ephemeral environments; amended 2026-08-14: the first-read keychain prompt is environment-conditional, and the AFK argument rests on the setup live-gate.
- **Written:** [`0030`](adr/0030-deferred-core-path-requires-documented-form-comparison.md) — a `deferred` core path makes the Reviewer's **Documented-Form Comparison** — a divergence list against the vendor's documented form, read in its own dispatch — required evidence, reported as its own outcome, never an automatic verdict flip.
- **Written:** [`0031`](adr/0031-skill-references-resolve-against-the-plugin-clone.md) — skill references resolve against the full-repo **Plugin clone** (pinned SHA); a reference is defective when written against the wrong **Resolution anchor** for its class, not when absent from the npm package — anchor discipline and dual-form invocations instead of inlining.
- **Written:** [`0032`](adr/0032-engine-invocation-is-a-setup-time-binding.md) — engine invocation is a single setup-time binding (`engine.cli` in the wave config), never a runtime fallback chain — the npm-first dual form had failed silently on five of seven rows.
- **Written:** [`0033`](adr/0033-issue-closing-work-lands-only-through-a-reviewed-wave-row.md) — issue-closing work lands only through a reviewed wave row; the **Coordinator-direct** lane is exclusively for session-authored orchestration artifacts that close no tracker issue — the test is binary.
- **Written:** [`0034`](adr/0034-a-rule-earns-its-enforcement-tier.md) — a rule earns its **Enforcement Tier** — prose is draft mode, structure production mode; a silently-failing rule becomes a **Promotion** candidate at its second live occurrence, and a Promotion pays its prose back to the residual form (amended 2026-08-13 and 2026-08-14: engine refusal outranks schema boundary when the condition names a field the same agent authors).
- **Written:** [`0035`](adr/0035-the-engine-cli-surface-is-the-third-semver-contract.md) — the engine CLI surface — verbs, flags, exit codes, JSON shapes — is the third semver contract beside the package root and the config schema.
- **Written:** [`0036`](adr/0036-the-archive-gate-is-claim-safe.md) — the archive gate is claim-safe: an awaiting-human row blocks `wave-close`'s archive, fail-closed, the structural twin of the disclosure gate.
- **Written:** [`0037`](adr/0037-an-adapter-owned-fact-is-imported-not-re-spelled.md) — an adapter-owned canonical fact is imported, not re-spelled, even against the layering grain — with the edge call-time-only so no module cycle forms.
- **Written:** [`0038`](adr/0038-a-find-is-captured-where-it-surfaces.md) — a find is captured where it surfaces: the Disclosure window ends at the archive, not at verdict-routing, and a wave-scoped Disclosure exists for a find no row owns.
- **Written:** [`0039`](adr/0039-user-directed-output-holds-the-operator-register.md) — user-directed output holds the **Operator register** (Convention 16): internal references translate into their one-line consequence at the audience boundary — strict in installed form, a compact pointer allowed in source form.
- **Written:** [`0040`](adr/0040-cross-skill-loading-is-a-sibling-path-read.md) — cross-skill loading is a sibling-path read against the loading skill's own base directory, not a by-name invocation — `disable-model-invocation` had made the by-name form unexecutable.
- **Written:** [`0041`](adr/0041-a-granted-scope-extension-travels-in-the-brief.md) — a granted **Scope extension** travels into the next round's brief as an explicit grant record, and every recompose re-fetches the spec; a grant spoken at routing cannot reach the round in flight.
- **Written:** [`0042`](adr/0042-the-sweep-owes-accounting-never-removal.md) — the **Sweep** owes accounting, never removal: what it cannot remove carries its **Survivor set** and **Manual recovery**, what it cannot see is reported **Unaccounted** — removal belongs to the Operator.
- **Written:** [`0043`](adr/0043-the-checked-text-never-carries-its-own-exemption.md) — the checked text never carries its own exemption: a **Subject path** is declared guard-side in a named, self-policing class, never marked inline in the content a guard reads.
- **Written:** [`0044`](adr/0044-a-goal-binds-a-native-container-and-derives-its-frontier.md) — a **Goal** binds one config-selected native container (Milestone · Project · Initiative · goal file) and derives its **Frontier** — sight, never permission: no dispatch path, no declared completion.
- **Written:** [`0045`](adr/0045-a-goals-members-are-the-containers-direct-native-members.md) — a Goal's members are the container's **direct native members** (issues for issue-direct containers, projects for a Linear Initiative), one frontier vocabulary at every member granularity — the transitive read was falsified against the live consumer.
- **Written:** [`0046`](adr/0046-the-mirror-pass-publishes-derived-accounting-to-the-containers-native-update-surface.md) — the **Mirror pass** publishes the derived Frontier as a native Project/Initiative Update — a narrative confirmed sentence-by-sentence, an engine-owned anchor, health only as a transcribed or source-attributed human judgment, never derived from the frontier.
- **Written:** [`0047`](adr/0047-a-stops-answer-is-a-typed-disposition-bound-to-a-spine-anchored-ask.md) — *(proposed)* a STOP's answer is a typed disposition bound to a spine-anchored **Ask**: engine-owned option sets per STOP reason, a sentinel-marked comment as the semver-governed wire form, tracker-derived authorization, four `human-*` events in `transition()`, one granted round per Ask.
- **Written:** [`0048`](adr/0048-a-headless-coordinator-is-a-finite-pulse-on-a-leased-spine-branch.md) — *(proposed)* a headless Coordinator is a finite **Pulse** on a leased `spine/<slug>` branch: publish-per-write inside the verbs, the Pulse-Log start entry pushed before any dispatch as the compare-and-swap **Lease** (expiry = the runner's own timeout), adopt-by-branch, flag-and-continue, a human-planned wave queue — never archives, never plans.
- **Written:** [`0049`](adr/0049-a-dispatched-agent-never-escalates-a-gates-capability-is-declared-provided-or-withheld.md) — a dispatched agent never escalates: a gate's **Capability requirement** is declared on the verify command, provided or withheld at dispatch, and a withheld gate leaves its acceptance criteria `capability-gated` — never a sandbox-off retry by an agent.
- **Written:** [`0050`](adr/0050-the-loaded-corpus-is-a-pinned-measure-and-a-rules-enforcement-tier-decides-its-reading-class.md) — three reading classes (**Standing load** · **Step load** · **Evidence**), a rule's **Enforcement Tier** decides which one its prose belongs in via a fixed `Enforced by:` line, and the standing load and the loaded corpus are pinned byte measures in the guard — lowered freely, raised only in a diff that says so; the plugin clone stays full.
- **Written:** [`0051`](adr/0051-a-verb-declares-its-own-contract-one-canonical-spelling-per-flag-silent-aliases-and-a-refusal-for-everything-undeclared.md) — every engine verb declares its own **Verb contract** (flags with one **Canonical spelling** and silent **Aliases**, positional arity, output class), the router aggregates them into the **Catalog**, and everything a contract does not declare is refused with exit 2 on every verb; canonical spellings come from the family rule and the `--<thing>-file` rule, one router-global `--json` replaces a `--receipt`, and no 3.0 is promised — the alias rows are the removal list a later major reads.

The first twenty-six ADRs (the nine zoom-out decisions + the to-issues-scope follow-on + the PRD-as-document decision + the `to-issues` grill's Worker redesign + the parent-is-an-opaque-id correction + the M1 #4 dor-non-file design + the triage-facet decision + the P7.3 spine-creation decision + the Linear Document-facet mapping + the P7.4 execution-driver decision + the P8 real-GitHubApi decision + the Linear-adapter mapping + the dispatch-log branch-home fix + the parked row state + the partial-arm landing decision + the Scribe sidecar-write decision + the amend-facet decision + the publication-cut decision) were written by the publication cut; ADRs 0027 through 0051 followed, grill by grill, and are indexed above — a `proposed` record is designed, not built, until its slices land. ADRs **0005 / 0006 / 0016** were amended by the P7.4 grill (re-work-loop owner + rejected-PR + rebase-train resolved; `needs-attention` write-path = an `IssueStore` facet; `briefProfile` dropped from the reviewer verdict); ADR **0015** was amended by the Linear-adapter grill (2026-07-10: on Linear the triage vocabulary is labels — the workflow states belong to the claim ledger); ADR **0016** was further amended by the dispatch-log branch-home fix (2026-07-15: an empty Dispatch-Log heading is not a write target — the `dispatch-log:` key must be scaffolded and `wave-start` must write it); ADRs **0005 / 0019** were amended by the auto-merge grill (2026-07-16: `--auto` = **partial-arm through the engine host seam** — `gh` leaves the creation and landing paths; ADR-0023); ADR **0018** was amended by the Scribe sidecar-write grill (2026-07-19: the driver's responsibility grows a persistence duty — sidecars are written by Scribe stages at agent-return; ADR-0024).
...
- True compare-and-swap claim (atomic, race-free) for many concurrent coordinators — M2 hardening; the small-team convention ("one coordinator dispatches at a time; humans treat `in-flight` as hands-off") suffices first. *Designed 2026-09-02 as the **Lease** — the Pulse-Log start entry pushed to `spine/<slug>` before any dispatch, a non-fast-forward rejection being the lost race ([ADR-0048](adr/0048-a-headless-coordinator-is-a-finite-pulse-on-a-leased-spine-branch.md)).*
