...
describe('skill-schema-drift — the re-dispatch section documents the checkout half-applying under the harness write-deny, and the driver actually carries it (issue #778)', () => {
  const driverMd = readFileSync(WORKFLOW_DRIVER_MD, 'utf-8');
  const driverJs = readFileSync(WORKFLOW_DRIVER_JS, 'utf-8');

  function redispatchSection(md: string): string {
    return contractRegion(
      md,
      'workflow-driver.md re-dispatch section',
  ...
  it('names the half-applied-checkout fact in prose, with the measured path and the live occurrence', () => {
    const region = redispatchSection(driverMd);
    expect(region).toMatch(/HALF-APPLY under the harness write-deny/);
    expect(region).toMatch(/skills corpus, `\.claude\/skills\/`, is the measured case/);
    expect(region).toMatch(/HEAD` lands on the re-dispatch branch correctly/);
    expect(region).toMatch(/2026-09-08-blocker-truth-and-call-form/);
  });

  it('states the refusal is a capability refusal and names staging through the git object store as not the path', () => {
    const region = redispatchSection(driverMd);
    expect(region).toMatch(/capability refusal/);
    expect(region).toMatch(/no retry with the sandbox off, no request for one/);
    expect(region).toMatch(/never by staging through the git object store, which the corpus guards cannot see/);
  });

  it('distinguishes the new clause from BOTH the harness-retry-of-iteration-1 case and the harness-retry-OF-a-re-dispatch case (issue #745)', () => {
    const region = redispatchSection(driverMd);
    expect(region).toMatch(/it is \*\*not\*\* a harness retry OF a re-dispatch either/);
    expect(region).toMatch(/issue #745/);
    expect(region).toMatch(/nothing about a retry changes which branch or commits are correct/);
  });

  it("states which clauses each iteration's brief carries, and that claim reads true against the rendered-brief spec", () => {
    const region = redispatchSection(driverMd);
    expect(region).toMatch(
      /The new clause's headline appears ONLY in the iteration≥2 \(re-dispatch\) brief/,
    );
    expect(region).toMatch(/the re-dispatch brief carries none of the iteration-1 inherited-work-in-progress clauses' headlines/);
    expect(region).toContain('tools/wave/src/compose-driver.spec.ts');
    // The claim is not just asserted in prose — the driver file it describes
    // actually carries the clause, at the section the doc says it lives in.
    expect(driverJs).toMatch(/IF THE CHECKOUT HALF-APPLIES UNDER THE HARNESS WRITE-DENY/);
    expect(driverJs).toMatch(/you may not re-run the checkout with the sandbox off/);
  });

  it('NEGATIVE CONTROL — the section fails loud when its start anchor is gone', () => {
    expect(() => redispatchSection('# nothing here\n')).toThrow(
      /contract region start anchor missing in workflow-driver\.md re-dispatch section/,
    );
  });

  it('NEGATIVE CONTROL — the capability-refusal + not-the-path assertions actually fire when that paragraph is stripped', () => {
    const stripped = driverMd.replace(
      /This is \*\*not\*\* the harness-retry case immediately below[\s\S]*?no request for one\.\n\n/,
      '',
    );
    expect(stripped).not.toEqual(driverMd); // the replace actually matched
    const region = redispatchSection(stripped);
    expect(region).not.toMatch(/capability refusal/);
    expect(region).not.toMatch(/issue #745/);
  });
});
