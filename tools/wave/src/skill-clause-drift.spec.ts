...
// ─── wave-resume step 5 levels with wave-close phase 4a's self-repair pull ───
//
// (issue #752) wave-close's phase 4a and wave-resume's step 5 both run the
// identical self-repair pull — the same write-denied-path check, the same
// Face-1/Face-2 leftover handling, the same three-step recovery — but only
// phase 4a was hardened live; the wave-resume copy drifted. This block pins
// the two level: the write-denied-path grep pattern must be byte-identical,
// and wave-resume must state phase 4a's three recovery steps in phase 4a's
// own order with no trace of the superseded `reset --hard origin/main`
// prescription.
describe("skill-clause-drift — wave-resume step 5 levels with wave-close phase 4a's self-repair pull (#752)", () => {
  const PHASE_4A_REL = '.claude/skills/wave-close/reference/phase-4a-self-repair-pull.md';
  const WAVE_RESUME_SKILL_REL = '.claude/skills/wave-resume/SKILL.md';
  const WAVE_RESUME_MECHANICS_REL = '.claude/skills/wave-resume/reference/resume-mechanics.md';

  const phase4a = SOURCES.get(PHASE_4A_REL) as string;
  const resumeSkill = SOURCES.get(WAVE_RESUME_SKILL_REL) as string;
  const resumeMechanics = SOURCES.get(WAVE_RESUME_MECHANICS_REL) as string;

  /**
   * The three-class write-denied-path grep pattern, exactly as phase 4a's
   * reference states it — the skills directory, the agents directory, and the
   * tracked settings file. A JS string, not a RegExp: the predicate below is a
   * literal-substring identity check between the two markdown bodies, not a
   * pattern match against either.
   */
  const DENIED_PATH_PATTERN = "'^\\.claude/(skills/|agents/|settings\\.json$)'";

  /**
   * The superseded prescription this row retires (the Gap this issue names:
   * wave-resume prescribed `reset --hard origin/main` where phase 4a prescribes
   * `reset --hard HEAD` plus targeted removal and re-pull). A literal substring
   * check, not a semantic one — the phrase must be gone byte-for-byte, not
   * merely reworded around.
   */
  const OLD_RESET_LINE = 'reset --hard origin/main';

  it("phase 4a really states the three-class pattern (the source of truth this block pins wave-resume to)", () => {
    expect(phase4a).toContain(DENIED_PATH_PATTERN);
  });

  it('wave-resume/SKILL.md states the identical write-denied-path pattern', () => {
    expect(
      resumeSkill,
      "wave-resume's pre-pull check has drifted from wave-close phase 4a's pattern — it must " +
        `grep against the identical three-class regex: ${DENIED_PATH_PATTERN}`,
    ).toContain(DENIED_PATH_PATTERN);
  });

  it('neither wave-resume file carries the superseded `reset --hard origin/main` prescription', () => {
    expect(resumeSkill).not.toContain(OLD_RESET_LINE);
    expect(resumeMechanics).not.toContain(OLD_RESET_LINE);
  });

  it('wave-resume points at phase 4a for the engine-surface list rather than keeping its own copy', () => {
    expect(resumeSkill).not.toMatch(/ENGINE_SURFACE\s*=/);
    expect(resumeSkill).toContain('phase-4a-self-repair-pull.md');
  });

  /**
   * Extract the Recovery block wave-resume's step 5 states, by its own anchor —
   * the same slice-by-anchor discipline `planReportPrescription` above uses: a
   * restructure that drops either anchor must FAIL this rather than silently
   * scan nothing or the whole file.
   */
  function extractRecoveryBlock(md: string): string {
    const start = md.indexOf('**Recovery —');
    if (start < 0) {
      throw new Error(
        "wave-resume's step-5 Recovery block heading is missing — this guard scopes itself by " +
          'that anchor and must not degrade to scanning an empty region.',
      );
    }
    const end = md.indexOf('Then the verify-clean tail', start);
    if (end < 0) {
      throw new Error("no 'Then the verify-clean tail' close after wave-resume's Recovery block");
    }
    return md.slice(start, end);
  }
  ...
  it('negative control — a seeded dropped-path-class divergence in EITHER copy fails the pattern-identity predicate', () => {
    const TWO_CLASS_PATTERN = "'^\\.claude/(skills/|settings\\.json$)'"; // the pre-#752 wave-resume pattern

    // (a) wave-resume's own copy regresses to the two-class pattern. `replaceAll`
    // (not `replace`) is load-bearing: every occurrence must be scrubbed, or a
    // second, untouched occurrence elsewhere in the same file would leave
    // DENIED_PATH_PATTERN still present and this negative control would prove
    // nothing.
    const droppedInResume = resumeSkill.replaceAll(DENIED_PATH_PATTERN, TWO_CLASS_PATTERN);
    expect(droppedInResume).not.toEqual(resumeSkill); // the replace actually matched
    expect(droppedInResume).not.toContain(DENIED_PATH_PATTERN);

    // (b) phase 4a's own copy regresses instead — the predicate must catch
    // drift starting from EITHER side, not only wave-resume's. Phase 4a states
    // the pattern TWICE (the code block and its own Common Mistakes echo), so
    // this is exactly the case `replaceAll` guards against.
    const droppedInPhase4a = phase4a.replaceAll(DENIED_PATH_PATTERN, TWO_CLASS_PATTERN);
    expect(droppedInPhase4a).not.toEqual(phase4a);
  ...
  it('negative control — restoring the old `reset --hard origin/main` line fails the absence predicate', () => {
    const regressed = `${resumeSkill}\n\ndebug fallback: git ${OLD_RESET_LINE} if unsure.\n`;
    expect(regressed).not.toEqual(resumeSkill);
    expect(regressed).toContain(OLD_RESET_LINE);
  });
});
