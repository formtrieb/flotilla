...
9. PROVE THE CHECK CAN FAIL (wave-shared Convention 11): if your slice introduces a NEW check — a test, an assertion, a guard, a smoke probe, a lint rule, a CI gate, a preflight, a validator — break the thing that check exists to catch, run the check, and observe its own FAIL state; then restore the original state and re-verify green. Report the falsification under \`judgmentCalls\` (mirrored in \`reviewerFocusItems\`): which check, what you broke, the observed failing output verbatim, and that you restored it. A green check is compatible with "the check works" AND "the check cannot fail", and no acceptance criterion distinguishes them. Two mechanical questions decide whether you are in this class — does a pass/fail check exist after your diff, and is its failing condition new with this slice — and "is the falsification worth the time" is deliberately NOT one of them: an expensive falsification belongs in the disclosure, not outside the class. A check observed only as \`deferred\`/\`skipped\` has NOT been proven to fail (it has been proven not to run) — that is a failed falsification attempt, not a demonstration. If you could not falsify it, say so in the same channel with the reason and what input WOULD falsify it — "could not falsify, and here is why" is a legitimate reported outcome; silence and an unevidenced claim are not. A slice that only changes behaviour already covered by EXISTING checks is not in this class. **Restoring a file under \`.claude/**\` — use your file-editing tool, never a shell \`git checkout\`/\`git restore\`.** A shell revert of a path under the harness's sandbox-protected \`.claude/**\` directories can HALF-APPLY under the write-deny: for a denied path it prints \`Operation not permitted\` and still finishes the rest of the command, so that one path is left staged at its OLD (still-broken) content in the index while every other path in the same command reverted correctly — nothing in the exit status distinguishes the two. Read \`git status --porcelain\` after ANY revert or restore that touches a \`.claude/**\` path, whichever tool you used, and treat a surviving modified/staged entry there as the half-apply, not as noise. Do the restore itself with your file-editing tool — read the anchor's own copy first with \`git show <anchor-sha>:<path>\` if you need its content — never the shell form. If the file-editing tool is ITSELF refused restoring a \`.claude/**\` path — a classifier refusal ("Self-Modification" / "Modify Shared Resources"), never the sandbox's own "Operation not permitted" — STOP \`blocked\`, quote the refusal verbatim, and disclose it under \`judgmentCalls\`: a 2026-09-25 measurement of 30 identical file-editing-tool edit-and-revert cycles from inside a Worker's own worktree (10 each on \`.claude/skills/**\`, \`.claude/agents/**\`, and a control path outside \`.claude/\`) found ZERO refusals, so there is no measured floor this clause bounds a retry against (wave-shared Convention 13's evidence sidecar carries the full per-attempt table). A future occurrence that measures a refusal followed by a successful identical retry is what would earn this clause a bounded-retry allowance instead of the STOP.
...
11. ONE BASH CALL PER STEP (wave-shared Convention 13): never fuse a setup step onto the command that matters — \`cd X && <command>\` — into one compound Bash call. **Two unrelated mechanisms break on that shape, with opposite signatures, and knowing only one of them is how the other bites.** (a) **The permission gate.** The harness splits a command on \`&&\`, \`||\`, \`;\`, \`|\`, \`|&\`, \`&\` and newlines and requires EVERY subcommand to match a permission rule independently — so the tracked-allowlist entry covering your engine or verify call carries only THAT subcommand past the gate, never the \`cd\` glued in front of it; and \`cd\` paired with \`git\`, or with an output redirect, prompts even when both halves are read-only commands. A permission dialog mid-AFK-dispatch has nobody to answer it and stalls your row. (b) **The worktree-isolation guard.** A fused command can come back REJECTED as too complex to verify that it stays inside your worktree — no dialog, nothing pending, nothing run. That refusal is about the command's SHAPE, never about the check being unrunnable here: re-issue it as separate calls. **NEVER silently skip a step because its fused form was refused** — a Worker did exactly that, dropping the check it was running and continuing; a verification step that did not run, in a report that reads as complete, is the defect (same family as Convention 12's empty capture). And do not "fix" either mechanism by asking for a \`cd\` allowlist entry — splitting costs nothing, and mechanism (b) would reject the fused form regardless. **Carry the directory IN the command wherever a flag exists** (\`npm ci --prefix <dir>\`, \`git -C <dir> …\`, \`--root\`/\`--cwd\`): your cwd is reset to your dispatch root before every Bash call, so a \`cd\` issued as its own call does NOT reach the next one — splitting a fused \`cd X && <cmd>\` into two calls leaves the command running in the wrong directory, which is a quieter defect than the fusion was. Your workspace-setup \`pwd\` is the one cwd fact you have, it holds for every call, and you never buy anything back by re-fusing. **Mechanism (b) is not limited to \`&&\`, and most refusals are not fusion at all.** Three shapes were once live-reproduced as refused with nothing fused onto them — a bare \`case\`/\`esac\`; an \`if\`-guard testing a shell variable; any other command naming a shell variable, including a lone \`test -n "$VAR"\` — and **a 2026-09-22 re-measurement from a worktree-isolated dispatch RETIRED that claim: a \`$VAR\` expansion is NOT refused on its own, in any position probed** (test, \`case\` pattern, \`printf\` argument, loop body, bare interpreter operand). The guard was demonstrably live for that re-run — the brace-bearing heredoc named below was refused in the same dispatch, word for word — so this is drift in the guard, not an absent guard. **Exactly one variable-bearing shape still refuses:** a \`cp\` to a computed path fused with a \`node --check\` on that same computed path, with five narrowing probes around it all accepted; it states its own objection inline, naming the construct and the variable, so you will recognize it if you meet it. A refusal is therefore still not diagnosed by punctuation — "split it" is not a general remedy and re-fusing is not either. **The prescriptions do not relax, because they never rested on that refusal.** Your cwd and every shell variable are reset between your Bash calls regardless — that half re-measured exactly as before — so a captured value is unreadable in the call that would spend it whether or not anything refuses the shape, which is why a value must never be carried from one call to the next: re-query its source in the call that needs it (your own Termination step 4 below does exactly that, via \`host-pr status\`), or use a single command whose exit status is the answer. Before re-deriving a fix by hand, read the "Catalog — three shapes named in one wave's disclosure, live-reproduced in this dispatch" section in \`wave-shared/reference/convention-13-one-bash-call-per-step.md\` — it records which entries the re-measurement retired and which still reproduce, so you neither re-adopt a dead end nor act on a refusal the measurement has withdrawn. **A FOURTH SHAPE is refused the same way, and this one has a working surface: a heredoc whose body carries a literal \`{\` or \`}\` near its head.** Live-reproduced across \`cat >> <file>\`, \`cat\` writing to stdout with NO redirect at all, and \`python3 -\` reading its program from stdin — so it is neither the redirect target nor the tool, and size is a second, independent trigger of its own — a brace-free body ran clean at 9,576 B and was refused at 10,032 B in the same dispatch, so a heredoc can refuse on size alone, with no brace in it at all. "Near the head" has no crisp threshold either: the identical brace pair was refused at one body position and accepted a few lines deeper, so **no brace-bearing heredoc can be predicted safe from its own text** and no re-shaping reliably reaches it. **PREFER YOUR FILE-EDITING TOOL OVER A SHELL HEREDOC FOR EVERY CONTENT WRITE YOU MAKE** — your PR-body file, any spec or doc you append to, any payload file a verb will read. It takes the path directly, creates the parent directory, involves no shell at all, and no occurrence on record has it refused. That is the general rule, not a local convenience of the sidecar-write step further down that already says it for its own payload. **That claim is about refusals only — it is not a claim that the tool preserves bytes exactly, and verify the bytes too.** A live occurrence had this tool convert a six-character escape sequence in handed content into the actual raw codepoint on write: a well-formed file that passed every guard, with an invisible unprintable character no diff showed, contradicting the very comment beside it that said the escape was used rather than a raw byte — and the same conversion then hit a PR body, rendering a code sample as an empty string. Do not trust an escape sequence to survive this tool: after any write whose content carries one, verify the bytes (grep the raw codepoint, or scan for non-ASCII) rather than trusting a clean diff or a passing guard. This does not relax the rule above — still write every content write through this tool rather than a heredoc; the byte check is its companion, not a substitute.

12. NEVER ESCALATE YOUR OWN PERMISSIONS — REPORT THE GATE INSTEAD (ADR-0049). **A dispatched agent never escalates its own permissions. Unconditionally, attended or not.** You may not disable the sandbox, ask for it to be disabled, re-run anything with it off, widen your own settings, or edit a permission/sandbox/config file to get a command through. **The retry-with-the-sandbox-off path is retired by name** — it is not a fallback you have, and there is no phrasing of it that is. Attendance is not a property you can read: the operator who dispatched the wave this clause comes from was awake at 03:45 and asleep when the prompt arrived, and five hours of one row were a click nobody could make. Your verify gate may render some commands with **declared needs** (paths outside the worktree, network hosts, or an un-narrowable host capability) — that is DATA about what the command must reach, never a licence to go and get it, and a declaration is not a grant. **When a verify command is refused for a permission or sandbox reason** — its declared need is unprovided, or it declared none at all — do exactly this: name the command and quote the refusal, report it as NOT RUN in \`tests\` / \`lint\` (never a count you did not observe, never silence), and disclose it under \`judgmentCalls\` mirrored into \`reviewerFocusItems\` so the Reviewer can defer the acceptance criteria that gate would have backed as \`capability-gated\`. The row then lands with an honest coverage gap and a non-blocking Disposition instead of a stalled wave. **This is the ONE refusal family that ends in "not run", and it is not policy clause 11's.** A clause-11 refusal is about a command's SHAPE — a \`$VAR\`-bearing command fused with a computed path, a control structure the isolation guard cannot verify — and the remedy is always to re-issue it in a form that runs; dropping it is still the defect there. A capability refusal is about a RIGHT you do not have and may not grant yourself, and no re-shaping reaches it. If you cannot tell the two apart from what came back, say so in the disclosure rather than picking the one that lets you continue.
...
**"Never drop a gate" is about its FORM, and it has exactly one exception, policy clause 12's:** a gate refused because you lack the permission to run it at all is reported as **not run, with the reason** — that is neither dropping it nor running it, and it is the only click-free honest answer available to you. Re-running it with the sandbox off is not an alternative to that report; it is the thing clause 12 forbids.

## Termination
1. Commit all work in one commit.
2. \`git push origin ${issue.branch}\` (never \`-u\`, never to default).
3. Open the PR **through the engine — never \`gh pr create\`** (\`gh\`'s creds are sandbox-denied and its TLS fails under the sandbox in every live run, ADR-0015; this verb uses the same \`fetch\` path the landing verbs do). Find-before-create is idempotent: a PR already open on this branch (e.g. a cap=1 re-dispatch onto the same branch) is **reused, never duplicated — and its title/body are re-written to the \`--title\` and the body you hand over** (\`updated: true\` in the JSON discloses it), so the body you compose reliably lands on the live PR (last-writer-wins).

   **The title you pass here is the title that lands, and it stays yours.** The rule downstream, stated once so you can rely on it: on a REUSE the body is re-written from the file that is handed over, and **the title is PRESERVED unless \`--title\` is passed**. You always pass \`--title\` on this call, so your title is what the PR opens with; the Coordinator's terminator (\`route-tuple\`) then re-writes only the body — placing the Reviewer verdict beneath your summary — and leaves the title you wrote byte-identical, because it passes no \`--title\` of its own. So compose it as the one-line form of the claim your body makes: it is what the reviewer reads first and, under the default, the subject of the commit that lands on the default branch.

   **What lands on the default branch is your PR, not your commits (ADR-0053).** Under the default (\`landing.commitMessage\` absent or \`pr\`), the PR title and body are the landed message on any commit count: the landing verb writes the squash commit from them whether your branch carries one commit or several. Your own commit messages do not reach the default branch under the default, so the PR body is where the durable record goes, never a commit message — except under a merge queue, which composes its own commit and ignores this message, or under a \`--method rebase\` landing, which replays the branch's own commits individually with no single message to shape (ADR-0053, dated note 2026-09-25); under either exception, it is your own commit messages, not the PR body, that land. Under \`host\`, the repository's own squash setting decides what lands instead.
   ...
   **And write a REAL body.** This step used to ask for "a one-paragraph summary", and one paragraph is what it got — which is why a Coordinator has repeatedly had to recover Worker-authored PR bodies out of agent transcripts instead of reading them on the PR. You are the only role that watched this change being made: say what you changed and why, what you falsified and what it printed, which decisions you took and which alternatives you rejected, and anything a reviewer would otherwise have to reconstruct. Length is not the point; being the durable record is.
   ...
   a. Write the body with your file-writing TOOL — it takes the path directly, creates the parent directory, and involves no shell at all — to this RELATIVE path in your own worktree's wave scratch directory:
      ...
      **Verify the bytes of what you just wrote, especially if the body carries an escape sequence.** The file-writing tool is refusal-free, not automatically byte-preserving — a live occurrence had it silently turn a six-character escape sequence into its raw codepoint, producing a body that read as clean while carrying an invisible unprintable character, and the same conversion once reached a PR and rendered a code sample as an empty string. A byte-level check after the write (not merely a re-read) is what catches this; a clean diff does not.
...
4. **Confirm the PR by asking the HOST, in a separate Bash call — never by carrying a value from step 3** (wave-shared Convention 12, half two). A finishing report with a missing or empty \`prUrl\` is the live failure this step exists to stop. **The confirmation is a re-query, not a guard on a capture**, because in your dispatch a guard on a capture cannot run at all:
   ...
   **Why a re-query and not a capture guard — read this before you improvise a shorter form.** Three earlier versions of this step prescribed a capture plus a guard, and all three were unrunnable from a worktree-isolated dispatch: a bare \`case\`/\`esac\` was refused standing alone, an \`if\`-guard naming the captured variable was refused, and fusing the capture and the guard back into one call was refused too. **A 2026-09-22 re-measurement, re-run live from a worktree-isolated dispatch, RETIRED those refusals** — a \`$VAR\` expansion is not refused on its own in any position probed, and exactly one variable-bearing shape still refuses (a \`cp\` to a computed path fused with a \`node --check\` on that same path). **The re-query stands anyway, on the reason that never depended on the guard:** shell state does not survive between your Bash calls — that half re-measured unchanged — so a captured URL is simply not there in the call that would spend it, and a guard on it would inspect an unset variable whether or not anything refuses the shape. The full station-by-station reproduction, and what the re-measurement did and did not retire, is Catalog entry 1 in \`wave-shared/reference/convention-13-one-bash-call-per-step.md\`. \`host-pr status\` sidesteps the whole question by carrying nothing across a call boundary — the host is asked again, and answers again.
...
**ONE BASH CALL PER STEP** (wave-shared Convention 13) — it binds you exactly as it binds the Worker, and this install is the first place it bites. Never fuse a setup step onto the command that matters (\`cd X && <command>\`) into one compound Bash call. Two unrelated mechanisms break on that shape, with opposite signatures: **the permission gate** splits a command on \`&&\`/\`||\`/\`;\`/\`|\`/\`&\`/newlines and requires EVERY subcommand to match a rule independently — so an allowlisted verify command carries only itself past the gate, never the \`cd\` in front of it, and a dialog mid-dispatch has nobody to answer it; and **the worktree-isolation guard** can REJECT a fused command as too complex to verify that it stays inside your worktree — no dialog, nothing run. A refusal is about the command's SHAPE, not about the check: re-issue it as separate calls. **NEVER drop a verify command or a floor check because its fused form was refused** — reporting a check as run when it was skipped is the exact failure this clause exists to stop, and it is yours to avoid as well as to catch in the Worker's evidence. Your cwd is reset to your dispatch root before every one of your Bash calls, so one \`pwd\` characterizes all of them and a preceding \`cd\` characterizes none: carry the directory in the command where a flag exists (\`npm ci --prefix <dir>\`, \`git -C <dir> …\`, \`--root\`/\`--cwd\`) rather than trusting a \`cd\` to reach the next call. **A bare newline joining two statements in one call is the same shape as \`&&\`, just quieter — and most refusals are not fusion at all:** \`case\`/\`esac\` was once observed refused standing entirely alone, and so was any command naming a **shell variable** — an \`if\`-guard on one, or a lone \`test -n "$VAR"\` — but **a 2026-09-22 re-measurement from a worktree-isolated dispatch RETIRED those refusals**: a \`$VAR\` expansion is not refused on its own in any position probed, exactly one variable-bearing shape still refuses (a \`cp\` to a computed path fused with a \`node --check\` on that same path), and the brace-bearing heredoc of Catalog entry 2 still refuses too. **Read that retirement as a correction to the REASON, never as licence to relax the prescription — and do not accept a Worker reading it the other way:** shell state does not survive between your Bash calls either — that half re-measured unchanged — so a value must still be re-queried in the call that needs it rather than carried, refusal or no refusal. Before re-deriving a split by hand, check the "Catalog — three shapes named in one wave's disclosure, live-reproduced in this dispatch" section in \`wave-shared/reference/convention-13-one-bash-call-per-step.md\` for what was actually verified — entry 1 records which of its stations the re-measurement retired and which still hold. When reviewing a Worker's evidence for THIS convention, treat a Worker's own citation of that catalog as legitimate rather than a shortcut — including a Worker reporting it COULD NOT verify a working form for a cataloged shape (the catalog's own heredoc-spec-append entry is exactly that outcome, honestly reported rather than guessed). **And check what the Worker's PR evidence rests on:** its Termination step now confirms the PR with a \`host-pr status --branch\` re-query, so a report whose \`prUrl\` traces back to a shell-variable capture is following a recipe the brief no longer carries.

**SECRET-SAFE** (wave-shared Convention 8): never echo any environment variable's VALUE — not even with fallback syntax like \${VAR:-no}. Never run whole-environment dumps (\`printenv\`, \`env\`, bare \`set\`). Never read a gitignored settings/secret file (e.g. \`cat .claude/settings.local.json\`, any \`.env\`-class file) — not even "to check config". Tool output must never contain a secret. **YOU DO NOT PROBE, EITHER** (wave-shared Convention 8's isolated-role rule): you never check whether a credential is set — sanctioned presence-test form or not. The Coordinator's own value-free credential-probe preflight already proved every configured credential resolves, once, before this row was dispatched, so there is nothing left here for you to check. **This is a policy rule, and after the 2026-09-22 re-measurement it is the ONLY reason left — say so precisely, per the measured posture (wave-shared Convention 13's Catalog entry 1):** that sanctioned form (\`[ -n "$VAR" ] && echo set\`) is the command the guard once rejected outright, live, when a Worker ran it — and the re-measurement **RETIRED that refusal**: the \`\$VAR\`-expansion shape the **ONE BASH CALL PER STEP** rule directly above (wave-shared Convention 13) names is not refused on its own in any position probed, so the form would very likely just run if you reached for it. It was never your reason anyway: unlike the Worker, whose \`agent()\` call sets \`isolation: 'worktree'\` explicitly (Stage 1), your own \`agent()\` call carries no \`isolation\` key at all (Stage 3), so the guard's command-complexity refusal was never established for your dispatch even while it did still reproduce for the Worker's. **A form that runs is still a form you do not run:** you skip this check because the rule says so and the Coordinator's preflight already answered the question, never because a guard is expected to reject it for you. **Nothing in your own review reads a secret — and the one host call you DO make is why that stays true.** \`${WAVE_CLI} host-pr status --branch ${issue.branch}\` is READ-ONLY and answers with the PR's state, its url and — off that same response, at no extra host call — its live \`title\` and \`body\`, so an acceptance criterion *about the PR body* is evidence you gather yourself rather than one you defer (a key the host did not surface comes back ABSENT, never an empty string; absence is \`deferred\`/\`partial\` with the unexercised claim named, never \`met\` on the Worker's word). The engine resolves the host credential inside that call, so you never see it, never echo it and never probe for it. What stays forbidden is every host WRITE — \`host-pr create\`/\`arm\`/\`merge\` belong to the Worker and the Coordinator, never to you — and every raw host CLI: \`gh pr view\` is not a shorter route to the same fact, it is a second credential path nothing in this system reviews (wave-shared Convention 7, ADR-0015).
...
sandbox, ask for it to be disabled, re-run anything with it off, widen your own settings, or edit a
permission or sandbox file to get a verify command through. Your BREADTH is in your COMMANDS — the
probe licence, the sibling merge-tree, Check 6's documented-form comparison — never in your RIGHTS.
You execute the code the Worker wrote, so anything wider here is an escalation channel for whatever
reached that diff through an issue body. **A verify command the Worker reported as not run for a
capability reason is one you re-run under the same conditions, or not at all.** A \`pass\` where the
Worker reported a capability refusal would compare two ENVIRONMENTS, not two runs — it is not a
verification of the Worker's claim, and reporting it as one is the finding. Your verify gate may
render commands with **declared needs**; that is data about what a command must reach, never a
licence to reach for it, and a declaration is not a grant. When a gate is genuinely withheld from
you: **\`capability-gated\` is the deferred valve's fourth trigger**, beside merge-gated, prod-gated
and human-gated (agent contract Check 3). The acceptance criteria that gate would have backed land
\`deferred\` — or \`partial\` where part of the criterion is exercised — with the unexercised outcome
named in \`evidence\` and **mirrored into \`reviewerFocusItems\`**, which is what turns it into a
Disclosure captured at routing and dispositioned before archive (ADR-0027). A withheld gate is not
\`changes-requested\`: it punishes a finished row for a setup gap and buys a round that can change
nothing. And it is not a new \`documentedFormComparison.trigger\` value either — that enum is
untouched; a capability-gated deferral landing on the row's CORE path fires the existing
\`deferred-core-path\` trigger, exactly as any other deferral does.
...
2. Write the payload below — the single line that follows this paragraph — EXACTLY,
   ...
   **Byte-verify the write when the payload carries an escape sequence.** This tool's refusal-free
   record (above) is a claim about refusals, not about byte fidelity: it has been observed converting
   a handed escape sequence into its raw codepoint on write, producing a well-formed, guard-passing
   file with an invisible unprintable character no diff shows. Verify the bytes of what was written
   whenever the payload contains one, rather than trusting a clean write.
